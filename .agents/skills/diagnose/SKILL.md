...
| `diagnose-hypothesise.j2` | `KnowAct` | Inline fallback for falsifiable-hypothesis generation (3–5, bug-form predictions) when `falsifiability` is unavailable. Primary path delegates step 3 to `falsifiability/falsifiability-hypothesize` (3–7, forced diversity, Platt-form predictions + falsifiers). |
...
- Hypothesis count: 3–7 when delegating to `falsifiability/falsifiability-hypothesize` (primary), or 3–5 via the inline `diagnose-hypothesise` fallback — every hypothesis must have a falsifiable prediction
