...
visibility: public
description: "Dynamic chain-of-thought reasoning engine with branching, revision, hypothesis testing, and automatic deep-dive delegation to specialized skills (hypothesis-framer, mcda, diagnose). Subsumes the deprecated sequential-thinking skill. The engine decides at runtime whether delegation is needed — no pre-selection. Templates: reasoning engine, three delegation targets (hypothesis-framer, mcda, diagnose), and a convergence check.
"
---
...
- A problem requires automatic deep-dive delegation to specialized skills (`hypothesis-framer`, `mcda`, `diagnose`) when the engine detects specific subproblems.
...
9. Emit a delegation request for a thought that needs analysis beyond reasoning alone, strictly limited to `hypothesis-framer`, `mcda`, or `diagnose`.
