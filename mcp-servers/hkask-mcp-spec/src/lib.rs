...
//!
//! The server struct and tool methods live here for fuzz testability (P5 Testing
//! Discipline). The binary target (`src/main.rs`) is a thin wrapper.
...
pub struct SpecServer {
    pub store: Arc<dyn SpecStore + Send + Sync>,
    ...
    pub event_sink: Arc<dyn hkask_types::event::NuEventSink>,
...
/// Run the spec MCP server (used by binary target).
pub async fn run(
    replicant: String,
