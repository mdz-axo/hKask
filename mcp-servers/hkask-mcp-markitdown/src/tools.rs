...
mod tests {
    ...
    /// Helper: create a file in CWD with a simple name (no `/`) that passes
    /// path validation. Returns the filename for use in MCP tool calls.
    /// Automatically cleaned up when dropped.
    struct TestFile {
        /// Simple filename (no directory component)
        name: String,
    }

    impl TestFile {
        fn create(content: &[u8], ext: &str) -> Self {
            use std::io::Write;
            let id = TEST_COUNTER.fetch_add(1, Ordering::Relaxed);
            let name = format!("hkask_test_{}.{}", id, ext);
            let mut f = std::fs::File::create(&name).expect("create test file");
    ...
    fn test_server() -> MarkitdownServer {
        MarkitdownServer::new(
            WebID::default(),
            Some("lighton-ocr-2-1b:latest".to_string()),
            "http://127.0.0.1:11435",
        )
        .expect("create server")
    }

    /// Parse the MCP tool response, extracting the `content` object from
    /// `McpToolOutput` wrapper. Returns the inner JSON value.
    fn extract_content(result: &str) -> serde_json::Value {
        let parsed: serde_json::Value = serde_json::from_str(result).expect("parse json");
        // ok_json wraps in {"content": ...}
        // error wraps in {"kind": ..., "message": ...}
    ...
    // ── Text extraction tests (no Okapi needed) ──────────────────────────

    #[tokio::test]
    async fn convert_plain_text() {
        let server = test_server();
        let f = TestFile::create(b"Hello world from plain text", "txt");

        let result = server
            .markitdown_convert(Parameters(ConvertRequest {
                path: f.name.clone(),
                force_ocr: false,
            }))
            .await;

        let data = extract_content(&result);
        assert_eq!(data["format"], "plain");
        assert_eq!(data["method"], "text_extraction");
        assert_eq!(data["word_count"], 5);
        assert!(data["text"].as_str().unwrap().contains("Hello world"));
    }

    #[tokio::test]
    async fn convert_markdown() {
        let server = test_server();
        let f = TestFile::create(b"---\nlayout: post\n---\n# Hello\nWorld", "md");
    ...
    async fn convert_html() {
        let server = test_server();
        let f = TestFile::create(
            b"<p>Hello <b>world</b></p><script>alert('x')</script>",
            "html",
        );
    ...
    #[tokio::test]
    async fn detect_format_various() {
        let server = test_server();
        ...
        let cases = [
            ...
            ("page.html", "html", true),
            ("notes.txt", "plain", true),
            ("report.docx", "docx", false),
            ("data.xlsx", "xlsx", false),
            ("unknown.xyz", "unknown", false),
        ];

        for (filename, expected_format, expected_supported) in cases {
            let result = server
                .markitdown_detect_format(Parameters(DetectFormatRequest {
                    path: filename.to_string(),
                }))
                .await;

            let data = extract_content(&result);
            assert_eq!(data["format"], expected_format, "format for {}", filename);
            assert_eq!(
                data["supported"], expected_supported,
                "supported for {}",
                filename
            );
        }
    ...
    async fn convert_unsupported_format_returns_error() {
        let server = test_server();
        let f = TestFile::create(b"test", "docx");
        ...
        let parsed: serde_json::Value = serde_json::from_str(&result).expect("parse json");
        // Error responses have "kind" and "message" fields
        assert!(parsed["kind"].is_string() || parsed["message"].is_string());
    }

    #[tokio::test]
    async fn convert_empty_file_returns_error() {
        let server = test_server();
        let f = TestFile::create(b"", "txt");
    ...
    // ── E2E OCR tests (require Okapi with vision model) ────────────────────
    //
