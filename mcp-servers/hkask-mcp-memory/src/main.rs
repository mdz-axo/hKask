...
use hkask_types::{McpErrorKind, Visibility, WebID, now_rfc3339};
...
async fn try_daemon_flow(replicant: &str) -> anyhow::Result<()> {
    ...
    let result = hkask_mcp::verify_startup_gates(&client, replicant, "memory", &[]).await?;
    tracing::info!(target: "hkask.mcp.memory", replicant = %replicant,
        "P4 gates verified{}",
