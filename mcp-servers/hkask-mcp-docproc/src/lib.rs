...
impl DocProcServer {
    ...
    #[tool(
        description = "Extract Kindle book pages from Kindle Cloud Reader. Logs in to Amazon, navigates to the book, captures PNG screenshots of each content page, and records metadata (title, author, TOC). Requires ASIN, Amazon credentials, and Chrome/Chromium installed. Idempotent — resumes from existing extraction if pages already exist on disk."
    )]
    pub async fn kindle_extract(
        &self,
        Parameters(params): Parameters<kindle_zip::KindleZipParams>,
    ) -> String {
        let span = ToolSpanGuard::new("kindle_extract", &self.webid);
        let asin = params.asin.clone();
        let email = params
            .amazon_email
            .ok_or_else(|| "amazon_email is required".to_string());
        let password = params
            .amazon_password
            ...
            .await
        {
            Ok(result) => {
                self.record_experience(
                    "kindle_extract",
                    &format!("ASIN={}, pages={}", asin, result.total_pages),
                    "success",
                    json!({"total_pages": result.total_pages, "content_pages": result.content_pages}),
                );
                span.ok_json(serde_json::to_value(&result).unwrap_or_default())
            }
            Err(e) => span.error(hkask_types::McpErrorKind::Unavailable, e),
        }
    }

    #[tool(
        description = "Transcribe Kindle book page screenshots to text using hKask's multi-backend OCR pipeline. Loads PNG pages from the extraction output, routes through Tesseract (simple pages) and LLM OCR (complex/fallback), and writes transcribed content to content.json with MDS provenance records. Requires HKASK_OCR_MODEL for vision-based OCR."
    )]
    pub async fn kindle_transcribe(
        &self,
        Parameters(params): Parameters<kindle_zip::KindleZipParams>,
    ) -> String {
        let span = ToolSpanGuard::new("kindle_transcribe", &self.webid);
        let asin = params.asin.clone();
        let output_dir = std::path::PathBuf::from(&params.output_dir);

        let pages_dir_str = params
            .pages_dir
            .unwrap_or_else(|| format!("output/{}/pages", asin));
        let metadata_path_str = params
            .metadata_path
            .unwrap_or_else(|| format!("output/{}/metadata.json", asin));
        let pages_dir = std::path::Path::new(&pages_dir_str);
        let metadata_path = std::path::Path::new(&metadata_path_str);

        let ocr_model = params.model.as_deref().or(self.ocr_model.as_deref());
        // Clone model string to avoid lifetime issues with embedding_router
        let embed_model = std::env::var("HKASK_EMBEDDING_MODEL")
            .unwrap_or_else(|_| "DI/Qwen/Qwen3-Embedding-0.6B".to_string());
        let embedding_router_ref: Option<(&hkask_inference::EmbeddingRouter, &str)> = self
            .embedding_router
            .as_ref()
            .map(|er| (er, embed_model.as_str()));

        match kindle_zip::transcribe_pages(
            pages_dir,
            metadata_path,
            &output_dir,
            &asin,
        ...
        .await
        {
            Ok(result) => {
                self.record_experience(
                    "kindle_transcribe",
                    &format!("ASIN={}, words={}", asin, result.total_words),
                    "success",
                    json!({
    ...
    #[tool(
        description = "Export assembled Kindle book content to PDF, EPUB, Markdown, and/or plain text. Generates format-specific files in the book's output directory. PDF uses Helvetica font with title page; EPUB is a valid EPUB 3.0 ZIP with XHTML chapters; Markdown includes TOC with heading anchors."
    )]
    pub async fn kindle_export(
        &self,
        Parameters(params): Parameters<kindle_zip::KindleZipParams>,
    ) -> String {
        let span = ToolSpanGuard::new("kindle_export", &self.webid);
        let asin = params.asin.clone();
        let output_dir = std::path::PathBuf::from(&params.output_dir);
        let metadata_path_str = params
            .metadata_path
        ...
        let toc: Vec<kindle_zip::TocItem> = std::fs::read_to_string(metadata_path)
            .ok()
            .and_then(|json| serde_json::from_str::<kindle_zip::BookMetadata>(&json).ok())
            .map(|m| m.toc)
            .unwrap_or_default();

        match kindle_zip::export_formats(
            &text,
            metadata_path,
            &params.formats,
            &output_dir,
            ...
            Ok(result) => {
                self.record_experience(
                    "kindle_export",
                    &format!("ASIN={}, formats={:?}", asin, result.exports.iter().map(|e| &e.format).collect::<Vec<_>>()),
                    "success",
                    json!({"total_bytes": result.total_bytes, "export_count": result.exports.len()}),
                );
                span.ok_json(serde_json::to_value(&result).unwrap_or_default())
            }
            Err(e) => span.error(hkask_types::McpErrorKind::Unavailable, e),
