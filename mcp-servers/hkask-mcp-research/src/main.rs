...
async fn main() -> Result<(), hkask_mcp::McpError> {
    ...
    let daemon_ok = match try_daemon_flow(&replicant).await {
        Ok(()) => true,
        Err(e) => {
            tracing::warn!(target: "hkask.mcp.research", replicant = %replicant, error = %e, "Daemon unavailable — falling back to direct mode");
            false
        }
    };
...
async fn try_daemon_flow(replicant: &str) -> anyhow::Result<()> {
    ...
    let result = hkask_mcp::verify_startup_gates(&client, replicant, "research", &[]).await?;
