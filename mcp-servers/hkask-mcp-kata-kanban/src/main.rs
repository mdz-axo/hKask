...
async fn main() -> Result<(), hkask_mcp::McpError> {
    ...
    let daemon_ok = match try_daemon_flow(&replicant).await {
        Ok(()) => true,
        Err(e) => {
            tracing::warn!(target: "hkask.mcp.kata_kanban", replicant = %replicant, error = %e, "Daemon unavailable — falling back to direct mode");
            false
        }
    };
    ...
    hkask_mcp_kata_kanban::run(replicant, daemon_client).await
}
...
async fn try_daemon_flow(replicant: &str) -> anyhow::Result<()> {
    ...
    tracing::info!(target: "hkask.mcp.kata_kanban", replicant = %replicant,
        "P4 gates verified{}",
