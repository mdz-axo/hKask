//! Baseten cloud training host.
use super::harness::HarnessAdapter;
use super::types::*;
...
impl TrainingHost for BasetenProvider {
    async fn submit(&self, job: &TrainingJob) -> Result<String, ProviderError> {
        // Resolve HuggingFace model ID and render training config via harness.
        let hf_model_id = crate::huggingface::resolve_model_id(&job.base_model);
        let mut resolved_job = job.clone();
        resolved_job.base_model = hf_model_id;
