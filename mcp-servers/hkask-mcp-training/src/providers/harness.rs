...
impl HarnessAdapter for AxolotlHarness {
    fn render_config(&self, job: &TrainingJob) -> Result<String, ProviderError> {
        ...
        // Prefer the HuggingFace dataset repository from artifacts (so the pod
        // pulls the dataset from HF at runtime). Fall back to the local path
        // only if artifacts are not set (e.g., local-only test paths).
        if let Some(ref artifacts) = job.artifacts {
            yaml.push_str(&format!("  - path: {}\n", artifacts.dataset.repository));
            if !artifacts.dataset.path.is_empty() {
                yaml.push_str(&format!("    data_files: {}\n", artifacts.dataset.path));
            }
