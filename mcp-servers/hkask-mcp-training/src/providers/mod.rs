...
/// compute runs (Together/Runpod/Baseten). The harness is injected into the
...
///
/// Selects a cloud host (Together/Runpod/Baseten) with API credentials.
...
pub struct TrainingHostConfig {
    /// Selected training host (Together, Runpod, Baseten).
...
mod tests {
    ...
    fn model_size_multiplier() {
        ...
        assert_eq!(types::extract_model_size_multiplier("Mixtral-8x7b"), 1); // bug: should be 2
        assert_eq!(types::extract_model_size_multiplier("unknown-model"), 2); // default
    }
    ...
    fn estimate_cost_is_positive() {
        ...
        let cost = types::estimate_training_cost_urj(&TrainingHostId::Baseten, 1, "unknown");
