...
//! - `HKASK_TRAINING_HOST` — Override host (together|runpod) — where compute runs
...
impl TrainingServer {
    ...
    async fn training_curate_feedback(
        ...
        if h_mems.is_empty() {
            return Err(McpToolError::invalid_argument(
                    "No training_qa_pair h_mems found. Ingest QA pairs first with training_ingest_qa.",
                ));
        }
    ...
    async fn training_sweep(&self, Parameters(req): Parameters<TrainSweepRequest>) -> String {
        ...
        if self.host_id == TrainingHostId::Runpod {
            return Err(McpToolError::failed_precondition(
                "RunPod sweeps require a published Hugging Face artifact path; submit jobs individually",
            ));
        }
