...
impl SqliteAdapterStore {
    ...
    pub fn new(db: hkask_storage::Database) -> Result<Self, AdapterStoreError> {
        let pool = db
            .sqlite_pool()
            .map_err(|e| AdapterStoreError::Storage(format!("pool: {e}")))?;
        Ok(Self { pool })
    }
    ...
    pub fn migrate(&self) -> Result<(), AdapterStoreError> {
        ...
        for column in [
            ...
            let exists = conn
                ...
                .map_err(|e| {
                    AdapterStoreError::Storage(format!("Inspect training_jobs schema: {e}"))
                })?
                .iter()
                .any(|existing| existing == name);
            if !exists {
                conn.execute_batch(&format!("ALTER TABLE training_jobs ADD COLUMN {column}"))
                    .map_err(|e| {
                        AdapterStoreError::Storage(format!("Add training_jobs.{name}: {e}"))
                    })?;
            }
        }
        Ok(())
...
mod tests {
    ...
    async fn sqlite_store_version_and_skill_name() {
        let db = setup_db();
        let store = SqliteAdapterStore::new(db).expect("store");
