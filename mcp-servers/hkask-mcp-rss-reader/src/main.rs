...
fn init_db(path: &str) -> Result<Connection, anyhow::Error> {
...
fn resolve_feed_url(conn: &Connection, stream_id: &str) -> Option<String> {
...
fn build_stream_where(stream_id: &str) -> (String, Vec<Box<dyn rusqlite::types::ToSql>>) {
...
fn mark_stream_read(conn: &Connection, stream_id: &str) -> Result<usize, anyhow::Error> {
    let (join_where, params) = build_stream_where(stream_id);
...
fn edit_tags(conn: &Connection, req: &EditTagRequest) -> Result<serde_json::Value, anyhow::Error> {
    let now = Utc::now().to_rfc3339();
