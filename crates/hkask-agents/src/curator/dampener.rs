...
mod tests {
    ...
    #[tokio::test]
    async fn dampener_suppresses_repeated_directive() {
        ...
        // Immediate repeat should be dampened
        assert!(dampener.should_dampen(&directive).await);
    }

    #[tokio::test]
    async fn dampener_allows_different_directive_same_type() {
        let dampener = Dampener::with_window(Duration::from_secs(60));

        let directive1 = CuratorDirective::CalibrateThreshold {
            domain: "variety".to_string(),
            new_threshold: 100,
        };
        let directive2 = CuratorDirective::CalibrateThreshold {
            domain: "tools".to_string(),
            new_threshold: 50,
        };

        assert!(!dampener.should_dampen(&directive1).await);
        // Different domain → different fingerprint → not dampened
        assert!(!dampener.should_dampen(&directive2).await);
    }

    #[tokio::test]
    async fn dampener_allows_different_directive_types() {
        let dampener = Dampener::with_window(Duration::from_secs(60));

        let calibrate = CuratorDirective::CalibrateThreshold {
            domain: "variety".to_string(),
            new_threshold: 100,
        };
        let update = CuratorDirective::UpdateCapabilities {
            agent: WebID::new(),
            additions: vec!["tool_a".to_string()],
            removals: vec![],
        };

        assert!(!dampener.should_dampen(&calibrate).await);
        // Different directive type → not dampened
        assert!(!dampener.should_dampen(&update).await);
    }

    #[tokio::test]
    async fn dampener_clear_resets_state() {
        let dampener = Dampener::with_window(Duration::from_secs(60));
        ...
        dampener.clear().await;

        // After clearing, the directive should not be dampened
        assert!(!dampener.should_dampen(&directive).await);
    }

    #[tokio::test]
    async fn dampener_does_not_dampen_non_governance_messages() {
        let dampener = Dampener::new();

        let message = LoopMessage::info(
            LoopOrigin::Observability,
            hkask_types::loops::dispatch::LoopPayload::Observation {
                category: "variety".to_string(),
                data: serde_json::json!({}),
            },
        );

        assert!(!dampener.should_dampen_message(&message).await);
    }

    #[tokio::test]
    async fn dampener_tracks_count_correctly() {
        let dampener = Dampener::with_window(Duration::from_secs(60));

        assert_eq!(dampener.tracked_count().await, 0);

        let d1 = CuratorDirective::CalibrateThreshold {
            domain: "variety".to_string(),
            new_threshold: 100,
        };
        let d2 = CuratorDirective::AdjustEnergyBudget {
            agent: WebID::new(),
            new_budget: 5000,
        };

        dampener.should_dampen(&d1).await;
        assert_eq!(dampener.tracked_count().await, 1);

        dampener.should_dampen(&d2).await;
        assert_eq!(dampener.tracked_count().await, 2);

        // Repeat of d1 doesn't add new entry
        dampener.should_dampen(&d1).await;
        assert_eq!(dampener.tracked_count().await, 2);
    }
}
