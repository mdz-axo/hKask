...
mod tests {
    ...
    fn test_decompose_imperative() {
        let analysis = decompose_prompt("Show me the latest results.");
        assert_eq!(analysis.imperative_count, 1);
    }

    #[test]
    fn test_decompose_conditional() {
        let analysis = decompose_prompt("If it rains, we should stay inside.");
        assert_eq!(analysis.conditional_count, 1);
    }

    #[test]
    fn test_depth_bucket() {
        let shallow = decompose_prompt("Hello. Goodbye.");
        assert_eq!(shallow.depth_bucket, "shallow");
    }

    #[test]
    fn test_topic_keywords_deduplication() {
        let analysis = decompose_prompt("The system runs. The system fails.");
        let system_count = analysis
            .topic_keywords
            .iter()
            .filter(|k| *k == "system")
            .count();
