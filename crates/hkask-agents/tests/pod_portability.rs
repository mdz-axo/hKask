...
async fn pod_portability_across_servers() {
    ...
    let salt_path = server_a
        ...
        .join("portable-alice")
        .join("pod.db.salt");
    // SQLite WAL companion files — may contain uncheckpointed data.
    let wal_path = server_a
        .path()
        .join("agents")
        .join("portable-alice")
        .join("pod.db-wal");
    let shm_path = server_a
        .path()
        .join("agents")
        .join("portable-alice")
        .join("pod.db-shm");
    ...
    // Copy SQLite WAL files if present (WAL journal mode).
    for w in [&wal_path, &shm_path] {
        if w.exists() {
            let name = w.file_name().unwrap();
            std::fs::copy(
                w,
                export_dir.join("agents").join("portable-alice").join(name),
            )
    ...
    if salt_path.exists() {
        std::fs::copy(
            &salt_path,
            pods_dir_b.join("portable-alice").join("pod.db.salt"),
        )
    ...
    for w in [&wal_path, &shm_path] {
        if w.exists() {
            let name = w.file_name().unwrap();
            std::fs::copy(w, pods_dir_b.join("portable-alice").join(name))
                .unwrap_or_else(|e| panic!("copy {} to B: {e}", name.to_string_lossy()));
        }
    }
