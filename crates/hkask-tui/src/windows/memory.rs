...
enum MemorySection { Episodic, Semantic, Triples, Consolidation }

impl MemorySection {
    fn next(self) -> Self { match self { Self::Episodic => Self::Semantic, Self::Semantic => Self::Triples, Self::Triples => Self::Consolidation, Self::Consolidation => Self::Episodic } }
    fn title(&self) -> &str { match self { Self::Episodic => "Episodic", Self::Semantic => "Semantic", Self::Triples => "Triples", Self::Consolidation => "Consolidation" } }
}
...
impl MemoryWindow {
    pub fn new(id: WindowId, bridge: Arc<dyn ReplBridge>) -> Self {
        Self { id, section: MemorySection::Episodic, bridge }
    }
}
...
impl Window for MemoryWindow {
    ...
    fn render(&self, f: &mut Frame, area: Rect, _focused: bool) {
        let mut lines = vec![
            Line::from(Span::styled(format!("── Memory: {} (Tab to switch) ──", self.section.title()), Style::default().fg(Color::Cyan).bold())),
            Line::from(""),
        ];
        match self.section {
            ...
            MemorySection::Semantic => {
                ...
                lines.push(Line::from("  CuratorPod SemanticIndex aggregates all pods."));
                lines.push(Line::from(""));
            ...
            MemorySection::Consolidation => {
                ...
                lines.push(Line::from("  Confidence floor: 0.33 (low-confidence triples removed)."));
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled("  Memory model: ν-events → episodic (private) → semantic (public) → SemanticIndex", Style::default().fg(Color::DarkGray))));
