...
impl Window for BackupWindow {
    ...
    fn title(&self) -> &str { "Backup" }
    fn kind(&self) -> WindowKind { WindowKind::Backup }

    fn render(&self, f: &mut Frame, area: Rect, _focused: bool) {
        let lines = vec![
            Line::from(Span::styled("── Backup Operations ──", Style::default().fg(Color::Cyan).bold())),
            Line::from(""),
            Line::from("  Commands:"),
            ...
            Line::from(""),
            Line::from(Span::styled("  Storage:", Style::default().fg(Color::DarkGray))),
            Line::from("    Location: ~/.config/hkask/backups/"),
            Line::from("    Format:   Encrypted SQLCipher (.db)"),
            Line::from(""),
            Line::from(Span::styled("  Use `kask backup` CLI for full functionality.", Style::default().fg(Color::DarkGray))),
        ];
