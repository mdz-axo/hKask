...
mod tests {
    ...
    async fn test_counter_metric() {
        let metric = CounterMetric::new("test_counter", "Test counter");
        metric.inc().await;
        metric.inc().await;

        let export = metric.export();
    ...
    async fn test_gauge_metric() {
        let metric = GaugeMetric::new("test_gauge", "Test gauge");
        metric.set(42.0).await;
    ...
    async fn test_histogram_metric() {
        let metric = HistogramMetric::new("test_histogram", "Test histogram", vec![1.0, 5.0, 10.0]);
        metric.observe(2.0).await;
        metric.observe(7.0).await;
    ...
    async fn test_metrics_registry() {
        ...
        let counter = registry.counter("test", "Test counter").await;
        counter.inc().await;
    ...
    #[tokio::test]
    async fn test_okapi_metrics_collector() {
        ...
        assert!(export.contains("hkask_retry_attempts_total"));
        assert!(export.contains("hkask_circuit_breaker_state"));
        assert!(export.contains("hkask_okapi_instances"));
    }
}
