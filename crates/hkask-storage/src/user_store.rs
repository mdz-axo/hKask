...
mod tests {
    ...
    fn test_logout() {
        let store = test_store();
        let request = RegistrationRequest {
            replicant_name: "dave".to_string(),
            first_name: "Dave".to_string(),
            last_name: "Test".to_string(),
            email: "dave@test.ai".to_string(),
            phone: None,
            passphrase: "DavePass123".to_string(),
        };
    ...
    fn test_encrypt_decrypt_pii_round_trip() {
        let key = UserStore::derive_pii_key("test-passphrase", &hex::encode([0u8; 16])).unwrap();
