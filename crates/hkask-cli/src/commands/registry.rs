...
/// post: removes the specified artifact from the registry; purges embeddings, h_mems, and disk artifacts
pub fn run_rm(
...
fn list_styles() {
    ...
    let pool = db.sqlite_pool().expect("Failed to create SQLite pool");
...
fn remove_style(artifact: &str, db_path: Option<String>, passphrase: Option<String>) {
    ...
    let triples_purged = crate::commands::helpers::or_exit(
        h_mem_store.delete_by_entity_prefix(&prefix),
        "Failed to purge h_mems",
    );
    ...
    eprintln!(
        "Purged {} embeddings and {} h_mems with prefix '{}'",
