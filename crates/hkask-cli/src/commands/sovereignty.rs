...
fn run_verify(action: SovereigntyAction) {
    ...
    if json {
        let result = hkask_services_core::verification::VerificationService::verify_json(
            principle.as_deref(),
        );
        println!(
            "{}",
    ...
    let report =
        hkask_services_core::verification::VerificationService::verify(principle.as_deref());
