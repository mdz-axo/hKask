//! Goal coordination commands — direct calls to goal repository.
//! Formerly delegated to GoalService (removed v0.31.0 per P5).

use hkask_types::GoalState;
use hkask_types::WebID;
use hkask_types::visibility::Visibility;

use crate::cli::GoalAction;
use crate::error::CliError;

/// Run a goal command.
pub fn run_goal(action: crate::cli::GoalAction) {
    let result = match action {
        GoalAction::Create { text, visibility } => create(&text, &visibility),
        GoalAction::List { state } => list(state.as_deref()),
        GoalAction::SetState { id, state } => set_state(&id, &state),
    };
    super::helpers::or_exit(result, "Goal command failed");
}

fn create(text: &str, visibility: &str) -> Result<(), CliError> {
    let ctx = super::helpers::build_agent_service();
    let owner = WebID::from_persona(b"cli-user");
    let vis = Visibility::parse_str(visibility).ok_or_else(|| {
        CliError::InvalidInput(format!(
            "Invalid visibility '{}': expected private | shared",
            visibility
        ))
    })?;
    let repo = ctx.storage().goals.clone();
    let goal = repo
        .create_goal(&owner, text, vis)
        .map_err(|e| CliError::AgentService(format!("Failed to create goal: {e}")))?;
    println!("Created goal {}", goal.id);
    println!("  text:       {}", goal.text);
    println!("  state:      {}", goal.state.as_str());
    println!("  visibility: {}", goal.visibility.as_str());
    Ok(())
}

fn list(state: Option<&str>) -> Result<(), CliError> {
    let ctx = super::helpers::build_agent_service();
    let owner = WebID::from_persona(b"cli-user");
    let filter =
        match state {
            Some(s) => Some(GoalState::parse_str(s).ok_or_else(|| {
                CliError::InvalidInput(format!("Invalid goal state filter '{}'", s))
            })?),
            None => None,
        };
    let repo = ctx.storage().goals.clone();
    let goals = repo
        .list_goals(&owner, filter)
        .map_err(|e| CliError::AgentService(format!("Failed to list goals: {e}")))?;
    super::helpers::print_item_list(&goals, "No goals found.", "Goals", |g| {
        format!("{} [{}] {}", g.id, g.state.as_str(), g.text)
    });
    Ok(())
}

fn set_state(id: &str, state: &str) -> Result<(), CliError> {
    let ctx = super::helpers::build_agent_service();
    let owner = WebID::from_persona(b"cli-user");
    let goal_id: hkask_types::id::GoalID = id
        .parse()
        .map_err(|_| CliError::InvalidInput(format!("Invalid goal ID '{}'", id)))?;
    let new_state = GoalState::parse_str(state)
        .ok_or_else(|| CliError::InvalidInput(format!("Invalid goal state '{}'", state)))?;
    let repo = ctx.storage().goals.clone();

    let goal = repo
        .get_goal(goal_id)
        .map_err(|e| CliError::AgentService(format!("Failed to load goal: {e}")))?
        .ok_or_else(|| CliError::InvalidInput(format!("Goal not found: {}", id)))?;

    // Ownership check — the only real logic that was in the service layer
    if goal.webid != owner {
        return Err(CliError::InvalidInput(
            "Not authorized to transition this goal".into(),
        ));
    }

    let from_state = goal.state.as_str().to_string();
    repo.update_goal_state(goal_id, new_state)
        .map_err(|e| CliError::AgentService(format!("Failed to update goal state: {e}")))?;

    // Curation inbox notification
    ctx.governance().notify_goal_transition(
        goal_id.to_string(),
        from_state,
        new_state.as_str().to_string(),
        WebID::from_persona(b"goal-service"),
    );

    println!("Goal {} -> {}", goal.id, goal.state.as_str());
    Ok(())
}
