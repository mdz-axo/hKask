...
use hkask_services_kata_kanban::{
    ColumnDef, KanbanService, TaskFilter, TaskSpec, TaskStatus, VerificationCriterion,
};
...
pub fn run_cli(action: KanbanAction, replicant_webid: WebID, _db_path: Option<&str>) {
    ...
    match action {
        ...
        KanbanAction::TaskCreate {
            ...
            if assign.is_some() {
                eprintln!("Tasks are created unassigned. The accepting agent must claim the task.");
            }
        ...
        KanbanAction::TaskAssign { task_id, agent } => {
            ...
            if agent_wid != replicant_webid {
                eprintln!("An agent must claim a task using its own authenticated WebID.");
                return;
            }
            match service.task_claim(tid, replicant_webid) {
                Ok(task) => {
                    println!(
                        "Task assigned to {}",
                        task.assignee
...
fn in_memory_store() -> HMemStore {
    let pool = hkask_database::sqlite::SqliteDriver::in_memory_pool().expect("in-memory pool");
