...
mod tests {
    ...
    #[test]
    fn test_bootstrap_phase_display() {
        assert_eq!(
            format!("{}", BootstrapPhase::Infrastructure),
            "infrastructure"
        );
        assert_eq!(format!("{}", BootstrapPhase::Security), "security");
        assert_eq!(format!("{}", BootstrapPhase::Mcp), "mcp");
        assert_eq!(format!("{}", BootstrapPhase::Bots), "bots");
        assert_eq!(format!("{}", BootstrapPhase::Curator), "curator");
        assert_eq!(
            format!("{}", BootstrapPhase::StandingSession),
            "standing_session"
        );
        assert_eq!(
            format!("{}", BootstrapPhase::KataReadiness),
            "kata_readiness"
        );
        assert_eq!(format!("{}", BootstrapPhase::CnsActive), "cns_active");
    }

    #[test]
    fn test_r7_bot_definitions_count() {
        let bots = BootstrapSequence::r7_bot_definitions();
        assert_eq!(bots.len(), 8, "Should have 8 R7 bot definitions");
    }

    #[test]
    fn test_r7_bot_definitions_have_webids() {
        let bots = BootstrapSequence::r7_bot_definitions();
        for bot in &bots {
            assert!(!bot.name.is_empty(), "Bot name should not be empty");
            assert!(
                !bot.allowed_spans.is_empty(),
                "Bot {} should have allowed spans",
                bot.name
            );
            assert!(
                bot.energy_budget > 0,
                "Bot {} should have positive energy budget",
                bot.name
            );
        }
    }
    ...
    #[test]
    fn test_kata_bot_in_definitions() {
        let bots = BootstrapSequence::r7_bot_definitions();
        let kata = bots.iter().find(|b| b.name == "kata-bot");
        assert!(kata.is_some(), "kata-bot should be in R7 definitions");
    }

    #[test]
    fn test_bot_webids_deterministic() {
        let bots1 = BootstrapSequence::r7_bot_definitions();
        let bots2 = BootstrapSequence::r7_bot_definitions();

        for (b1, b2) in bots1.iter().zip(bots2.iter()) {
            assert_eq!(
                b1.webid, b2.webid,
                "WebID for {} should be deterministic",
                b1.name
            );
        }
    }
    ...
    fn test_bootstrap_state_default() {
        ...
        assert!(state.bot_webids.is_empty());
        assert!(state.curator_webid.is_none());
        assert!(state.session_id.is_none());
    }

    #[test]
    fn test_error_phase_mapping() {
        let err = BootstrapError::Infrastructure("test".into());
        assert_eq!(err.phase(), BootstrapPhase::Infrastructure);

        let err = BootstrapError::CnsActivation("test".into());
        assert_eq!(err.phase(), BootstrapPhase::CnsActive);
    }

    #[tokio::test]
    async fn test_bootstrap_runs_all_phases() {
        let cns_runtime = Arc::new(CnsRuntime::new());
        let mut bootstrap = BootstrapSequence::new(cns_runtime);
        let result = bootstrap.run().await;
        assert!(result.is_ok(), "Bootstrap should complete successfully");

        let state = bootstrap.state();
        assert_eq!(
            state.completed_phases.len(),
            8,
            "All 8 phases should complete"
        );
        assert!(state.curator_webid.is_some(), "Curator WebID should be set");
        assert!(state.session_id.is_some(), "Session ID should be set");
        assert_eq!(
            state.bot_webids.len(),
            8,
            "All 8 bot WebIDs should be recorded"
        );
    }

    #[tokio::test]
    async fn test_bootstrap_kata_readiness_fails_without_kata_bot() {
        // This test verifies the kata-bot is present in definitions
        let bots = BootstrapSequence::r7_bot_definitions();
        let kata = bots.iter().find(|b| b.name == "kata-bot");
        assert!(
            kata.is_some(),
            "kata-bot must be present for bootstrap to succeed"
        );
    }
}
