...
mod tests {
    ...
    fn trace_id_roundtrip() {
        let id = TraceId::new();
        let s = id.to_string();
        let id2 = TraceId::from_string(&s);
        assert_eq!(id, id2);
    }

    #[test]
    fn message_priority_ordering() {
        assert!(MessagePriority::Critical.order() < MessagePriority::Warning.order());
        assert!(MessagePriority::Warning.order() < MessagePriority::Info.order());
    }

    #[test]
    fn loop_message_creation() {
    ...
    fn loop_message_with_target() {
        ...
        .with_target(LoopOrigin::Governance);

        assert!(msg.is_directed());
        assert_eq!(msg.target_loop, Some(LoopOrigin::Governance));
    }
}
