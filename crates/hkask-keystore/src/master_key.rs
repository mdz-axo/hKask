...
mod tests {
    //
    // Behavioral tests for master key derivation.
    //
    ...
    //   Domain       → derive_sub_key determinism, domain separation, length
    //   Composition  → InternalSecrets field independence, hex encoding
    //   Curation     → derive_all_internal_secrets determinism
    //

    use super::*;

    // Helper: derive a 32-byte master key once via Argon2id (slow ~100ms).
    // Reuse across tests that only need derive_sub_key to avoid repeated Argon2 calls.
    fn master_key_for_test() -> Zeroizing<Vec<u8>> {
        use crate::encryption::derive_key;
        let test_salt = [0xAB; crate::encryption::SALT_SIZE];
        let key = derive_key("test-master-pass", &test_salt)
            .expect("derive_key for test helper should succeed");
        Zeroizing::new(key.to_vec())
    }

    // ── Domain: derive_sub_key determinism ──────────────────────────

    // P8 invariant: same master key + same context → same sub-key
    #[test]
    fn derive_sub_key_is_deterministic() {
        let mk = master_key_for_test();
        let sub1 = derive_sub_key(&mk, "test-context");
        let sub2 = derive_sub_key(&mk, "test-context");
        assert_eq!(*sub1, *sub2, "same inputs must produce identical sub-keys");
    }

    // P8 invariant: same master key + different context → different sub-keys
    #[test]
    fn derive_sub_key_domain_separation() {
        let mk = master_key_for_test();
        let sub_a = derive_sub_key(&mk, "context-a");
        let sub_b = derive_sub_key(&mk, "context-b");
        assert_ne!(
            *sub_a, *sub_b,
            "different contexts must produce different sub-keys"
        );
    }

    // P8 invariant: sub-keys are always 32 bytes
    #[test]
    fn derive_sub_key_output_length_is_32_bytes() {
        let mk = master_key_for_test();
        let sub = derive_sub_key(&mk, "len-check");
        assert_eq!(sub.len(), 32, "sub-key must be exactly 32 bytes");
    }

    // ── Composition: derive_all_internal_secrets ─────────────────────

    // Call Argon2id once and reuse the result for the next 3 tests.
    // This is the single slow helper — Argon2id runs exactly once.
    fn secrets_for_test() -> InternalSecrets {
        derive_all_internal_secrets("test-passphrase")
    }

    // P8 invariant: same passphrase → same secrets
    #[test]
    fn derive_all_internal_secrets_is_deterministic() {
        let s1 = secrets_for_test();
        let s2 = secrets_for_test();
        assert_eq!(
            s1.acp_secret, s2.acp_secret,
            "acp_secret must be deterministic"
        );
        assert_eq!(
            s1.capability_key, s2.capability_key,
            "capability_key must be deterministic"
        );
        assert_eq!(
            s1.mcp_security_key, s2.mcp_security_key,
            "mcp_security_key must be deterministic"
        );
        assert_eq!(
            s1.ocap_secret, s2.ocap_secret,
            "ocap_secret must be deterministic"
        );
    }

    // P8 invariant: each secret field differs from the others
    #[test]
    fn derive_all_internal_secrets_fields_are_distinct() {
        let s = secrets_for_test();
        let fields = [
            &s.acp_secret,
            &s.capability_key,
            &s.mcp_security_key,
        ...
        for i in 0..fields.len() {
            for j in (i + 1)..fields.len() {
                assert_ne!(
                    fields[i], fields[j],
                    "secret fields must be distinct (pair [{i},{j}] matched)"
                );
            }
        }
    }

    // P8 invariant: each field is a valid 64-char hex string (256 bits)
    #[test]
    fn internal_secrets_fields_are_hex_encoded_64_chars() {
        let s = secrets_for_test();
        for (name, value) in [
            ("acp_secret", &s.acp_secret),
            ("capability_key", &s.capability_key),
            ("mcp_security_key", &s.mcp_security_key),
            ("ocap_secret", &s.ocap_secret),
        ] {
            assert_eq!(
                value.len(),
                64,
                "{name} must be 64 hex chars (256 bits), got {} chars",
                value.len()
            );
            assert!(
                value.chars().all(|c| c.is_ascii_hexdigit()),
                "{name} must be valid hex, got: {value}"
            );
        }
    }
}
