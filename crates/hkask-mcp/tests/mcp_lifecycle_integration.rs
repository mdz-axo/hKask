...
// ── Per-tool required-capability (blast-radius split) ─────────────────────
//
// Locks the filesystem read/write/execute blast-radius declarations that the
// GovernedTool membrane enforces via the action hierarchy (Execute ≥ Write ≥
// Read). See BUILTIN_TOOL_CAPS in hkask_mcp::lib.

#[test]
fn capability_for_tool_filesystem_read_tools_require_read() {
    use hkask_mcp::capability_for_tool;
    for tool in ["fs_read", "fs_list", "fs_search"] {
        assert_eq!(
            capability_for_tool("filesystem", tool).as_deref(),
            Some("tool:filesystem:read"),
            "{tool} must require read"
        );
    }
}

#[test]
fn capability_for_tool_filesystem_write_tools_require_write() {
    use hkask_mcp::capability_for_tool;
    for tool in ["fs_write", "fs_edit", "fs_delete"] {
        assert_eq!(
            capability_for_tool("filesystem", tool).as_deref(),
            Some("tool:filesystem:write"),
            "{tool} must require write"
        );
    }
}

#[test]
fn capability_for_tool_shell_exec_requires_execute() {
    use hkask_mcp::capability_for_tool;
    assert_eq!(
        capability_for_tool("filesystem", "shell_exec").as_deref(),
        Some("tool:filesystem:execute"),
        "shell_exec must require execute"
    );
}

#[test]
fn capability_for_tool_falls_back_to_server_id_derivation() {
    use hkask_mcp::capability_for_tool;
    // A tool with no per-tool override derives from the server id.
    assert_eq!(
        capability_for_tool("memory", "recall_episodic"),
        hkask_capability::capability_from_server_id("memory"),
        "tools without an override fall back to server-id derivation"
    );
}
