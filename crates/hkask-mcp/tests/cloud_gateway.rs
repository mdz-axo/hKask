...
async fn cloud_gateway_round_trip() {
    ...
    // ── Client mTLS + token ──────────────────────────────────────────
    let client_cert_der =
        rustls_pemfile::certs(&mut std::io::BufReader::new(client_cert_pem.as_bytes()))
            .collect::<Result<Vec<_>, _>>()
            .unwrap();
    let client_key_der =
        rustls_pemfile::private_key(&mut std::io::BufReader::new(client_key_pem.as_bytes()))
            .unwrap()
            .unwrap();
    let mut ca_store = rustls::RootCertStore::empty();
    for c in rustls_pemfile::certs(&mut std::io::BufReader::new(ca_pem.as_bytes()))
        .collect::<Result<Vec<_>, _>>()
        .unwrap()
    {
