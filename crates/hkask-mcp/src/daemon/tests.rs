...
fn request_variants_serialize_to_correct_shape() {
    ...
    let drift = serde_json::to_value(DaemonRequest::SpecDriftQuery {
        replicant: "alice".into(),
        spec_id: Some("spec-1".into()),
    })
    .unwrap();
    assert_eq!(drift["type"], "spec_drift_query");
}
...
fn missing_optional_fields() {
    ...
    // Spec ID is optional
    let json4 = r#"{"type":"spec_drift_query","replicant":"bob"}"#;
    let req4: DaemonRequest = serde_json::from_str(json4).unwrap();
    match req4 {
        DaemonRequest::SpecDriftQuery { spec_id, .. } => assert!(spec_id.is_none()),
        _ => panic!("wrong variant"),
    }
}
