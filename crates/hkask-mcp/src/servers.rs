...
mod tests {
    ...
    async fn test_sqlite_storage() {
        ...
        storage
            .write("key1".to_string(), serde_json::json!("value1"))
            .await;
        storage
            .write("key2".to_string(), serde_json::json!("value2"))
            .await;

        let value = storage.read("key1").await;
        assert_eq!(value, Some(serde_json::json!("value1")));

        let keys = storage.list(None).await;
        assert_eq!(keys.len(), 2);

        let keys = storage.list(Some("key")).await;
        assert_eq!(keys.len(), 2);
    ...
    #[tokio::test]
    async fn test_register_builtin_servers() {
        let runtime = McpRuntime::new();
        register_builtin_servers(&runtime).await;

        let servers = runtime.list_servers().await;
        assert_eq!(servers.len(), 2);

        let tool_count = runtime.tool_count().await;
        assert!(tool_count >= 8); // 4 storage + 4 registry tools
    }
}
