...
mod tests {
    ...
    fn setup() -> (TempDir, GixCasAdapter) {
        let tmp = TempDir::new().expect("create temp dir");
        let adapter = GixCasAdapter::new(tmp.path()).expect("create adapter");
        (tmp, adapter)
    }

    // Tracer bullet 3: put_blob stores content and returns a BLAKE3 hash
    #[tokio::test]
    async fn put_blob_returns_deterministic_blake3_hash() {
        let (_tmp, adapter) = setup();
        let repo = RepoId::Registry;

        let hash1 = adapter
            .put_blob(&repo, b"hello world")
            .await
            .expect("put_blob should succeed");
        let hash2 = adapter
            .put_blob(&repo, b"hello world")
            .await
            .expect("put_blob should succeed again");
        ...
        let hash3 = adapter
            .put_blob(&repo, b"different content")
            .await
            .expect("put_blob should succeed");
        assert_ne!(
            hash1, hash3,
    ...
    // Tracer bullet 4: get_blob retrieves content put earlier
    #[tokio::test]
    async fn get_blob_retrieves_content_put_earlier() {
        let (_tmp, adapter) = setup();
        let repo = RepoId::Registry;

        let hash = adapter
            .put_blob(&repo, b"test data")
            .await
            .expect("put_blob should succeed");
    ...
    // Tracer bullet 9: path traversal is blocked
    #[test]
    fn validate_path_rejects_traversal() {
        assert!(GixCasAdapter::validate_path(Path::new("../../etc/passwd")).is_err());
        assert!(GixCasAdapter::validate_path(Path::new("/absolute")).is_err());
        assert!(GixCasAdapter::validate_path(Path::new("normal/file.txt")).is_ok());
        assert!(GixCasAdapter::validate_path(Path::new("a\0b")).is_err());
    }

    // Tracer bullet 10: RepoId isolation
    #[tokio::test]
    async fn repo_id_isolation_put_blob_in_different_repos() {
        let (_tmp, adapter) = setup();

        let hash_registry = adapter
            .put_blob(&RepoId::Registry, b"registry data")
            .await
            .expect("put_blob in Registry");

        let hash_memory = adapter
            .put_blob(&RepoId::Memory, b"memory data")
            .await
            .expect("put_blob in Memory");

        // Content is isolated per repo
        let registry_content = adapter
            .get_blob(&RepoId::Registry, &hash_registry)
            .await
            .expect("get_blob from Registry");
        assert_eq!(registry_content, b"registry data");

        let memory_content = adapter
            .get_blob(&RepoId::Memory, &hash_memory)
            .await
            .expect("get_blob from Memory");
        assert_eq!(memory_content, b"memory data");

        // Cross-repo access should fail
        let result = adapter.get_blob(&RepoId::Memory, &hash_registry).await;
        assert!(
            result.is_err(),
            "hash from Registry should not be found in Memory"
        );
    }

    // Tracer bullet 8: verify detects corruption
    #[tokio::test]
    async fn verify_detects_corrupt_blob() {
        let (tmp, adapter) = setup();
        let repo = RepoId::Registry;

        let hash = adapter
            .put_blob(&repo, b"original content")
            .await
            .expect("put_blob should succeed");
        ...
        let cas_path = tmp
            ...
            .join(hash.to_string());
        if cas_path.exists() {
            std::fs::write(&cas_path, b"corrupted!!!").expect("corrupt blob");
        }

        let report = adapter.verify(&repo).await.expect("verify should succeed");
        assert!(
            !report.corrupt_hashes.is_empty(),
