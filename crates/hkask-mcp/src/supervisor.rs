...
mod tests {
    ...
    async fn test_supervisor_start_stop() {
        ...
        supervisor.register(config).await;
        supervisor.start("test-server").await.unwrap();

        let status = supervisor.status("test-server").await.unwrap();
        assert!(status.running);
        assert!(status.pid.is_some());

        supervisor.stop("test-server").await.unwrap();
    ...
    async fn test_supervisor_restart() {
        ...
        let status1 = supervisor.status("test-server").await.unwrap();
        let pid1 = status1.pid.unwrap();

        supervisor.restart("test-server").await.unwrap();

        let status2 = supervisor.status("test-server").await.unwrap();
        let pid2 = status2.pid.unwrap();

        // PID should be different after restart
        assert_ne!(pid1, pid2);
        assert_eq!(status2.restart_count, 1);
    }
}
