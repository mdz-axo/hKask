...
mod tests {
    ...
    fn test_adapter_container_configure() {
        let container = AdapterContainer::new();
        let temp_path = env::temp_dir().join("hkask_adapter_test");
    ...
    fn test_adapter_container_clear() {
        let container = AdapterContainer::new();
        let temp_path = env::temp_dir().join("hkask_adapter_clear_test");
    ...
    fn test_adapter_container_thread_safety() {
        ...
        for i in 0..10 {
            ...
            let handle = thread::spawn(move || {
                let temp_path = env::temp_dir().join(format!("hkask_thread_{}", i));
                std::fs::create_dir_all(&temp_path).unwrap();
