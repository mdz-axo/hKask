...
mod tests {
    ...
    #[test]
    fn empty_manifests_produces_empty_report() {
        let report = build_report(&[], None);
        assert_eq!(report.total_assertions, 0);
        assert_eq!(report.total_pass, 0);
        assert_eq!(report.total_fail, 0);
    }

    #[test]
    fn filter_matches_principle() {
        ...
        let report = build_report(&[manifest], Some("p1"));
        assert_eq!(report.principles.len(), 1);
    }

    #[test]
    fn absence_check_vacuously_true() {
        let assertion = Assertion {
            id: "a1".to_string(),
            name: "test".to_string(),
            claim: "".to_string(),
            method: "absence_check".to_string(),
            targets: vec![],
            prohibited: vec![],
        };
        let result = verify_absence_check(&assertion);
        assert_eq!(result.status, "pass");
    }

    #[test]
    fn unknown_method_is_gap() {
        let assertion = Assertion {
            id: "a1".to_string(),
            name: "test".to_string(),
            claim: "".to_string(),
    ...
    fn json_output_has_escalation() {
        ...
        let report = build_report(&[manifest], None);
        let json = serde_json::json!({
            "total_fail": report.total_fail,
            "total_gap": report.total_gap,
            "escalation_required": report.total_fail > 0 || report.total_gap > 0,
        });
        assert_eq!(json["escalation_required"], true);
    }
}
