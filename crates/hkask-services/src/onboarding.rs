...
mod tests {
    ...
    // REQ: svc-onb-001 — derive_and_store_secrets returns valid ResolvedSecrets
    #[test]
    fn derive_and_store_secrets_returns_secrets() {
        // We can't test keychain storage in CI (no keyring backend),
        // but we can test that derive_secrets produces non-empty values.
        let secrets = OnboardingService::derive_secrets("test-passphrase-123");
        assert!(
            !secrets.acp_secret.is_empty(),
            "acp_secret should be non-empty"
        );
        assert!(
            !secrets.db_passphrase.is_empty(),
            "db_passphrase should be non-empty"
        );
    }

    // REQ: svc-onb-002 — derive_secrets produces deterministic results
    #[test]
    fn derive_secrets_is_deterministic() {
        let a = OnboardingService::derive_secrets("consistent-input");
        let b = OnboardingService::derive_secrets("consistent-input");
        assert_eq!(a.acp_secret, b.acp_secret, "same input → same acp_secret");
        assert_eq!(
            a.db_passphrase, b.db_passphrase,
            "same input → same db_passphrase"
        );
    }

    // REQ: svc-onb-003 — derive_secrets produces different results for different inputs
    #[test]
    fn derive_secrets_differs_for_different_inputs() {
        let a = OnboardingService::derive_secrets("passphrase-alpha");
        let b = OnboardingService::derive_secrets("passphrase-beta");
        assert_ne!(
            a.acp_secret, b.acp_secret,
            "different inputs → different acp_secret"
        );
    }

    // REQ: svc-onb-004 — try_list_existing_replicants returns empty for missing DB
    #[test]
    fn try_list_returns_empty_for_missing_db() {
        let config = ServiceConfig::in_memory();
        let result = OnboardingService::try_list_existing_replicants(&config);
        assert!(
            result.is_empty(),
            "in-memory config with no DB should return empty"
        );
    }

    // REQ: svc-onb-005 — remove_orphaned_db returns false for in-memory DB
    #[test]
    fn remove_orphaned_returns_false_for_in_memory() {
        let config = ServiceConfig::in_memory();
        assert!(
            !OnboardingService::remove_orphaned_db(&config),
            "in-memory DB should not be orphaned"
        );
    }

    // REQ: svc-onb-006 — cleanup_failed_onboarding does not panic for in-memory config
    #[test]
    fn cleanup_does_not_panic_for_in_memory() {
        let config = ServiceConfig::in_memory();
