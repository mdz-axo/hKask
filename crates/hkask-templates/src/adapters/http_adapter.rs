...
impl HttpAdapter for MockHttpAdapter {
    async fn execute(&self, _request: HttpRequest) -> Result<HttpResponse, HttpAdapterError> {
        if self.should_fail {
            Err(HttpAdapterError::RequestFailed("Mock failure".to_string()))
        } else {
...
mod tests {
    ...
    async fn test_mock_adapter() {
        ...
        let mock = MockHttpAdapter::new(response.clone());
        let result = mock
            .execute(HttpRequest::get("http://test.com".to_string()))
            .await;
    ...
    async fn test_mock_adapter_failure() {
        let mock = MockHttpAdapter::failing();
        let result = mock
            .execute(HttpRequest::get("http://test.com".to_string()))
            .await;
