...
mod tests {
    ...
    fn test_parse_skill() {
        ...
        let skill = pipeline.parse_skill("test", "claude_skill");
        assert_eq!(skill.name, "Imported Skill");
    }

    #[test]
    fn test_map_to_template_prompt() {
        ...
        let pipeline = SkillTranslationPipeline::new();
        let template = pipeline.map_to_template(&skill);
        assert_eq!(template.template_type, TemplateType::Prompt);
    }

    #[test]
    fn test_map_to_template_process() {
        let skill = ParsedSkill {
            id: "test".to_string(),
            name: "Test".to_string(),
            description: "Test skill".to_string(),
            prompts: vec![],
            steps: vec![Value::String("step1")],
            capabilities: vec![],
        };
        let pipeline = SkillTranslationPipeline::new();
        let template = pipeline.map_to_template(&skill);
        assert_eq!(template.template_type, TemplateType::Process);
    }
    ...
    fn test_energy_calculation() {
        ...
        let config = EnergyCalculation {
            base_cost: 1000,
            per_prompt_cost: 200,
            per_step_cost: 300,
            complexity_multiplier: HashMap::new(),
        };
        let energy = calculate_energy(&skill, &config);
        assert_eq!(energy, 1500); // 1000 + 2*200 + 1*300
    }
}