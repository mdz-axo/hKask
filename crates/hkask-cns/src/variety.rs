...
mod tests {
    ...
    fn test_variety_counter_increment() {
        ...
        assert_eq!(counter.get("state_a"), 2);
        assert_eq!(counter.get("state_b"), 1);
        assert_eq!(counter.variety(), 2);
    ...
    fn test_variety_counter_deficit() {
        ...
        assert_eq!(counter.deficit(5), 3); // Expected 5, got 2
        assert_eq!(counter.deficit(2), 0); // Expected 2, got 2
        assert_eq!(counter.deficit(1), 0); // Expected 1, got 2 (saturating)
    }
    ...
    fn test_variety_counter_entropy() {
        ...
        // Equal distribution should have max entropy for 2 states
        let entropy = counter.entropy();
        assert!((entropy - 1.0).abs() < 0.001); // log2(2) = 1

        counter.increment("a");
        counter.increment("a");
        // Skewed distribution should have lower entropy
        let entropy2 = counter.entropy();
        assert!(entropy2 < entropy);
    }

    #[test]
    fn test_variety_monitor() {
        let mut monitor = VarietyMonitor::new();
        monitor.counter("domain_a").increment("x");
        monitor.counter("domain_b").increment("y");
        monitor.counter("domain_b").increment("z");

        assert_eq!(monitor.domains().len(), 2);
        assert_eq!(monitor.counter("domain_a").variety(), 1);
        assert_eq!(monitor.counter("domain_b").variety(), 2);
    }
    ...
    fn test_variety_monitor_total_deficit() {
        ...
        monitor.counter("b").increment("y");

        // Each domain has 1 variety, expected 3 per domain
        // Deficit = (3-1) + (3-1) = 4
        assert_eq!(monitor.total_deficit(3), 4);
    }
}
