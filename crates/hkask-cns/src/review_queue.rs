...
impl ReviewQueue {
        ...
        match decision {
            ReviewDecision::FalsePositive => {
                ...
                if let Some(block) = self
                    .blocks
                    .iter_mut()
                    .find(|b| b.violation_id == violation_id)
                {
                    block.deactivate();
    ...
    pub fn is_agent_blocked(&self, agent_id: &WebID) -> bool {
        self.blocks
            .iter()
            .any(|b| b.agent_id == *agent_id && b.active && !b.is_expired())
    }

    pub fn get_agent_block(&self, agent_id: &WebID) -> Option<&TemporaryBlock> {
        self.blocks
            .iter()
            .find(|b| b.agent_id == *agent_id && b.active && !b.is_expired())
    }
...
mod tests {
    ...
    fn test_violation_review() {
        ...
        assert!(violation.reviewed);
        assert_eq!(
            violation.review_decision,
            Some(ReviewDecision::FalsePositive)
        );
    ...
    fn test_violation_severity_block_duration() {
        assert_eq!(
            ViolationSeverity::Warning.default_block_duration(),
            Duration::minutes(0)
        );
        assert_eq!(
            ViolationSeverity::Moderate.default_block_duration(),
            Duration::minutes(15)
        );
        assert_eq!(
            ViolationSeverity::Severe.default_block_duration(),
            Duration::hours(2)
        );
        assert_eq!(
            ViolationSeverity::Critical.default_block_duration(),
            Duration::hours(24)
        );
    }
