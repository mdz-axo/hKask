...
mod tests {
    ...
    #[test]
    fn known_server_returns_configured_cost() {
        let est = TableGasEstimator::new();
        assert_eq!(est.lookup("hkask-mcp-ocap", "any_tool"), 1);
        assert_eq!(est.lookup("hkask-mcp-fal", "any_tool"), 100);
    }

    #[test]
    fn unknown_server_returns_default() {
        let est = TableGasEstimator::new();
        assert_eq!(est.lookup("unknown-server", "any_tool"), 10);
    }

    #[test]
    fn per_tool_cost_overrides_server_cost() {
        let est = TableGasEstimator::new();
        // condenser server = 10, but thread_summary tool = 25
        assert_eq!(est.lookup("hkask-mcp-condenser", "compress"), 10);
        assert_eq!(
            est.lookup("hkask-mcp-condenser", "condenser_thread_summary"),
            25
        );
    }

    #[test]
    fn inference_server_table_cost_is_zero() {
        let est = TableGasEstimator::new();
        // Inference cost is 0 in table — overridden by InferenceGasEstimator
        assert_eq!(est.lookup("hkask-mcp-inference", "any_tool"), 0);
    }

    #[test]
    fn gas_tier_ordering() {
        let est = TableGasEstimator::new();
        // Internal < Local I/O < Moderate < External < Heavy
        let internal = est.lookup("hkask-mcp-ocap", "x");
        let local = est.lookup("hkask-mcp-spec", "x");
        let moderate = est.lookup("hkask-mcp-condenser", "x");
        let external = est.lookup("hkask-mcp-web", "x");
        let heavy = est.lookup("hkask-mcp-fal", "x");
        assert!(internal < local);
        assert!(local < moderate);
        assert!(moderate < external);
        assert!(external <= heavy);
    }

    #[test]
    fn default_impl_matches_new() {
        let a = TableGasEstimator::new();
        let b = TableGasEstimator::default();
        // Both should return same cost for the same lookup
        assert_eq!(
            a.lookup("hkask-mcp-ocap", "x"),
            b.lookup("hkask-mcp-ocap", "x")
        );
    }
}
