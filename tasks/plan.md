...
- `AgentPersona`/`AgentCharter`/`AgentIdentity` (`pod/types.rs:111-150`). **Delete** (userpods have services/preferences, not charters) — open Q-PERSONA: keep A2A presentation fields?
...
- **P10 — Bot/Replicant Taxonomy** (`PRINCIPLES.md:206`). **Retire** (open Q-P10: retire vs replace with userpod principle).
...
- **One measurable target (4–8 weeks):** zero `Agent*` persona types, zero `AgentKind` variants,
  `PodKind` 3→2 (Curator, UserPod), `is_primary`=gone, P10 retired, P12 retitled, persistent
  userpods, curator as systemd daemon, `cargo build --workspace` green, `userpod_*` tests green.
...
| S-6 | Delete `AgentPersona/Charter/Identity` (keep A2A presentation? Q-PERSONA) | PASS | n/a | n/a | **DELETE** (Q-PERSONA) | Guideline |
...
| S-12 | Retire P10; edit P5.2/P6/P6.1/P9/P12 + mandate doc + comments + skills | PASS — naming a deleted concept | n/a | n/a | **EDIT** | Guardrail (spec: principles in scope) |
...
- **T2.2** Delete `AgentPersona`/`AgentCharter`/`AgentIdentity` (Q-PERSONA: keep A2A presentation fields in UserPod?). Remove persona-YAML path. — *Acc: no Agent* persona types; ≤4 files*
...
- **T4.2** Collapse `PodLifecycleState` → persistent + A2A-registered-on-start (≤2 states); remove `Deactivated`/teardown. — *Acc: ≤2 states; tests green; ≤3 files; checkpoint: User*
...
```json
{
  ...
  "target_metric_after": {
    ...
    "P10_BotReplicantTaxonomy": "retired (or replaced per Q-P10)",
...
- R4: P10 retirement reduces Twelve→Eleven principles — semantic/numbering consequence. **Guardrail**: gate T5.2 on Q-P10.
...
## 10. Open questions (remaining after Phase 0)

- Q-AK: delete `AgentKind` entirely (A2A takes no kind) vs keep single `UserPod` variant? (lean delete — G3)
- Q-PERSONA: UserPod A2A presentation — keep name+version, drop charter/identity-YAML? (lean yes)
- Q-P10: retire P10 (Eleven Principles) vs replace with "P10 — UserPod Agency"? (user decision)
- Q-SPAN: rename CNS span field `replicant_host`→`userpod_host` or `host`? (affects skill YAML + CNS canonical namespace check)
- Q6: full consumer trace of `AgentPod`/`PodDeployment` (T2.0).
- Q-LIFE-FINAL: persistent pods — is "register-on-start" the only lifecycle event, or do userpods have a "suspended" state for inactive users?
...
| S-8 | `pod/types.rs:57-66` | Guardrail (SIMPLIFY) | 1 (lifecycle states 4→≤2) | persistent pods; register-on-start; no Deactivated | medium |
...
| S-12 | `PRINCIPLES.md:128,206,214`, mandate doc | Guardrail (EDIT) | 1 (P10 retired; P6/P12 retitled) | principles name userpods not replicant/bot | cheap |