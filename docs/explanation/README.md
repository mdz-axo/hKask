...
| [Regulation and Loops](cns-and-loops.md) | Regulation homeostatic loop (sense→compare→compute→act→verify), skill PDCA model (FlowDef manifests, convergence contracts), Curator metacognition (CurationLoop + MetacognitionLoop), bug hunting methodology (Weinberg, Beizer, Hendrickson), QA system (YAML manifests, LLM classification, Regulation spans). | Core |
