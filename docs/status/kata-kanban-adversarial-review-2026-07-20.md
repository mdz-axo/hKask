...
| 3 | `task_consume_gas`/`task_consume_rjoules` never called | **WIRED** — added `TaskGasAccountant` trait + `KanbanTaskGasAccountant` impl + `KataEngine::with_task_gas_accountant` builder + `KanbanService::gas_accountant_for(task_id)` + CLI `--task` flag on `kask kata start`. Each inference call now deducts actual token usage from the bound task's `gas_remaining`. 4 new integration tests verify the loop. |
