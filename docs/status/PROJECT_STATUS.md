...
|---------|------|-----------|
| `GoalCapabilityToken` HMAC omitted `operations` and `expires` → forgeable authority | Security | `GoalCapabilityToken` removed in v0.23.00 — goal operations now use WebID-based owner scoping |
| Goal write paths verified the token but not the holder's ownership → confused deputy | Capability | `GoalCapabilityToken` removed in v0.23.00; every write enforces `GoalAccess::can_write`/`can_admin` against the holder WebID |
