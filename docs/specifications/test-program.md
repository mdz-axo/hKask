...
Capability tokens for test operations follow the same `DelegationToken` pattern as spec operations (see [`MDS.md`](../architecture/MDS.md)). The `spec:validate` action covers test verification.
