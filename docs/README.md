...
| [`corpus_inventory.yaml`](status/corpus_inventory.yaml) | lifecycle, curation | Document corpus lifecycle classification (generated 2026-06-14) |

> **Pending infrastructure:** `spec-code-drift.yaml` and `curation-decisions.yaml` are referenced by MDS_SCAFFOLD and README but not yet created. These will be generated by the `hkask-mcp-spec` server's drift detection and curation decision operations.
