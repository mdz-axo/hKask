...
> **Curator-owned secrets** (`CONDUIT_MATRIX_SIGNING_KEY`) are stored in the Curator's keystore, not in per-pod Fly Secrets. They are set once during `kask curator init` and never exposed to pod configuration.
