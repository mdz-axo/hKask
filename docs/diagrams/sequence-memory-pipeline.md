...
| No EAV match | Seed new semantic triple | Decayed episodic confidence |
| Episodic triple expired | Soft-delete via `valid_to` | Source removed from episodic |
...
sequenceDiagram
    ...
    rect rgb(245, 248, 252)
        ...
        Daemon->>+Epi: record_experience() → store(triple)
        ...
        alt visibility is Shared or Public
            ...
            Note over Epi: "Episodic memory is sovereign"<br/>— shared/public triples rejected
        else perspective is None
            Epi-->>Epi: EpisodicMemoryError::MissingPerspective
        else valid Private + perspective
            Epi->>+Sq: triple_store.insert(&triple)
            Sq-->>-Epi: Ok(())
    ...
    rect rgb(245, 252, 245)
        ...
        alt trigger threshold reached
            ...
            Epi->>+Epi: dedup_triples() — EAV hash dedup
    ...
    rect rgb(255, 252, 240)
        ...
        Note over Epi: Selects oldest/lowest<br/>effective-confidence triples
        Epi-->>-Bridge: Vec<Triple> (candidates)

        loop each candidate triple
            ...
            alt EAV match found (combine)
                ...
                Sem-->>-Bridge: Ok(())
                Note over Bridge: Bayesian combined —<br/>existing semantic triple updated
            else No EAV match (seed)
                Sem-->>-Bridge: None
                Bridge->>+Bridge: new semantic triple:<br/>· stripped perspective (None)<br/>· visibility → Shared/Public<br/>· confidence = episodic_c
                Bridge->>+Sem: store_consolidated(triple)
                Sem-->>-Bridge: Ok(())
                Note over Bridge: New semantic triple seeded
            end
    ...
    rect rgb(248, 245, 255)
        ...
        opt confidence_floor specified
            Svc->>+Sem: low_confidence_triples(floor, MAX)
            loop each low-confidence triple
                Svc->>+Sem: delete_triple(id)
            end
        end

        opt max_semantic_triples specified
            Svc->>+Sem: triple_count()
            alt count > max
                Svc->>+Sem: lowest_confidence_triples(count - max)
                loop each lowest-confidence triple
                    Svc->>+Sem: delete_triple(id)
                end
            end
        end
    ...
    rect rgb(245, 248, 252)
        ...
        alt consent denied
            ...
            Sq-->>-Epi: triples (decayed + deduped)
        end
        ...
        Sq-->>-Sem: triples (deduped by EAV hash)
        Note over Sem: recall_dedup::dedup_triples(filtered)
    end
```
...
| Dedup strategy | `recall_dedup::dedup_triples()` | `recall_dedup::dedup_triples()` |
| Confidence at read | Wozniak-Gorzelanczyk decay applied | Stored confidence as-is |
...
verified_against: >
  ...
  crates/hkask-memory/src/semantic.rs:61-130 (SemanticMemory, store, query_deduped, store_consolidated),
  crates/hkask-memory/src/consolidation.rs:26-168 (ConsolidationBridge, consolidate, Bayesian combine),
  crates/hkask-memory/src/consolidation_service.rs:10-100 (ConsolidationService, consolidate, cleanup),
  crates/hkask-memory/src/recall_dedup.rs:10-57 (eav_hash, dedup_triples, BLAKE3),
...
| [`SemanticMemory`](crates/hkask-memory/src/semantic.rs:61-130) | Shared, public memory with similarity-augmented recall |
...
| [Magna Carta P1](docs/magna-carta.md#p1-user-sovereignty) | User Sovereignty — episodic memory as sovereign first-person |
