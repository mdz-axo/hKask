...
> **2026-07-07 Documentation Initiative:** Completed documentation items (Diataxis restructuring, how-to guides, explanation docs, reference API docs, diagram generation) have been moved to the archive. The new documentation structure is indexed at [`docs/README.md`](../README.md). See `docs/status/documentation-inventory-2026-07-07.md` for the full deliverable list.
...
|----|------|-------|----------|--------|----------|
| **P0-01** | CNS span emission integration | CNS bot | High | ✅ Complete | CNS spans distributed across `crates/hkask-cns/src/governed_tool.rs`, `crates/hkask-types/src/event.rs`; core registry: `crates/hkask-types/src/cns.rs` (`CnsSpan`, 7 core variants); domain spans in `hkask-wallet/src/cns_span.rs` (`WalletSpan`), `hkask-federation/src/cns_span.rs` (`FederationSpan`), `hkask-cns/src/cns_span.rs` (`CnsDomainSpan`); all namespaces registered in `CANONICAL_NAMESPACES` (event.rs) |
| **P0-02** | Git CAS integration for hMems | Storage bot | High | ✅ Complete | Deleted in 2026-06-10 architecture audit (see HANDOFF.md) |
...
| **P1-11** | Energy-use tracking simplification + security hardening | Dev + CNS | High | ✅ Complete | Rename (Energy* → Gas*). Persistence (save/load). Escalation (algedonic pathway). Stale reservation auto-release. Consumption velocity. Well + Wallet system with SQLite persistence + auto-draw. Status: `docs/status/gas-budget-system-status.md`. |
...
| **P2-08** | MDS audit R6: Consolidate CNS span listings (3→1 authoritative source) | Curator | Medium | ✅ Complete | canonical CNS span registry: `CANONICAL_NAMESPACES` in `crates/hkask-types/src/event.rs`; core spans in `CnsSpan` (7 variants), domain spans in `WalletSpan`, `FederationSpan`, `CnsDomainSpan` |
...
| **C-22** | End-to-end verification — Hemingway corpus embedded via DeepInfra | 2026-06-12 | 1832 embeddings, centroid stored, 28,989 hMems — full pipeline completes |
...
| **C-41** | R5: CnsSpan enum (72+ → 7 core variants) | 2026-07-05 | Core `CnsSpan` reduced to 7 cross-cutting variants (Tool, Inference, AgentPod, Gas, Curation, SelfHeal, MemoryEncode). Domain-specific spans migrated to `WalletSpan` (hkask-wallet), `FederationSpan` (hkask-federation), `CnsDomainSpan` (hkask-cns). CANONICAL_NAMESPACES is the single registry. `SpanNamespace::from_observable()` bridge added. |
