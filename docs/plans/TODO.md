...
| **P1-11** | Energy-use tracking simplification + security hardening | Dev + CNS | High | 📋 Partial | Rename complete (Energy* → Gas*). Persistence save implemented, load not wired. Escalation emits tracing but not connected to Curator algedonic pathway. Full gap inventory in `docs/status/gas-budget-system-status.md` (2026-07-01). 8 gaps decomposed into atomic steps. |
