...
`active`. Stale `reg.supply_chain.*` "gap noted" language cleaned up across
...
|-------|----------|--------|-------------------|
| `kali-audit` | `registry/templates/kali-audit/` | Enforced (`security/regressions/` active) | P5 minimal, P8 semantic, P9 Regulation (`reg.*` spans), 8-layer defense catalog |
...
| `supply-chain-sentinel` | `registry/templates/supply-chain-sentinel/` (new) | `active` (user-accepted 2026-07-18; was `pending` per `skill-logic-audit` `user-choice`) | P4 OCAP (manifest-only, no external download), P8 evidence-backed (`CWE-1104/829/1357`, `OWASP Supply Chain`, `OSC&R`), P9 `reg.supply_chain.*` registered in `CANONICAL_NAMESPACES` (`event.rs` L295-299), P12 `userpod_host` mandatory |
| `runtime-posture-monitor` | `registry/templates/runtime-posture-monitor/` (new) | `active` (registry-committed 2026-07-18) | P4 OCAP (runtime Regulation telemetry only, no external endpoint scanning), P8 evidence-backed (`CWE-1357/829/200`, `OWASP LLM06/07`, `ATLAS AML.TA0010`), P9 `reg.runtime.*` registered in `CANONICAL_NAMESPACES` (`event.rs` L302-308), P12 `userpod_host` mandatory, 6-layer runtime defense catalog (distinct from kali-audit's 8 static layers) |
| `attack-taxonomy-mapper` | `registry/templates/attack-taxonomy-mapper/` (new) | `active` (registry-committed 2026-07-18) | P4 OCAP (consumes existing findings, no external taxonomy download), P8 evidence-backed (`CWE-1104/829/1357`, `OWASP Supply Chain`, `OSC&R`), P9 `reg.taxonomy.*` registered in `CANONICAL_NAMESPACES` (`event.rs` L309-315), P12 `userpod_host` mandatory, backward-compatible `taxonomy_mapping` field |
...
- `.agents/skills/supply-chain-sentinel/SKILL.md` — updated P9 Regulation regulation
  section, convergence-check step 6, and Constraints to state all four
  `reg.supply_chain.*` namespaces are registered and emitted unconditionally.
...
- `docs/plans/runtime-posture-monitor-design.md` — full design spec for
  `runtime-posture-monitor` skill: 5W1H gate, P5 deletion test, Regulation namespace
  proposal (`reg.runtime.*` — pre-registered), 4-template decomposition,
  defense-layer catalog, convergence metric, open questions.
- `docs/plans/attack-taxonomy-mapper-design.md` — full design spec for
  `attack-taxonomy-mapper` skill: 5W1H gate, P5 deletion test, Regulation namespace
  proposal (`reg.taxonomy.*` — pre-registered), 4-template decomposition,
...
- `registry/templates/runtime-posture-monitor/` — manifest.yaml + 4 .j2
  ...
  convergence-check) + SKILL.md. All `reg.runtime.*` spans emitted
...
- `registry/templates/attack-taxonomy-mapper/` — manifest.yaml + 4 .j2
  ...
  + SKILL.md. All `reg.taxonomy.*` spans emitted unconditionally
...
userpod_host mandatory, P8 evidence-backed, P9 Regulation spans emitted
unconditionally.
...
- **5W1H gate check:** Who = running application / userpod host; What = API endpoint exposure / bot detection / LLM usage; Where = runtime environment / production workload; When = continuous (not audit cycle); Why = `P3.1` safe container requires runtime blocking (`Aikido` `Zen` firewall model: block attacks without code change); How = observe runtime signals (`reg.runtime.*` — registered in `CANONICAL_NAMESPACES`) → classify threat patterns → emit regulation events (`reg.regulation`) → trigger defensive action (`reg.guard.violation`).
- **P5 minimal test:** Does NOT download external packages; does NOT replace endpoint detection (`Huntress` — zero overlap); reads only runtime telemetry (`hkask.*` performative spans) and produces `reg.runtime.*` canonical spans (`P9`).
...
**Status:** `active` — registry committed (2026-07-18). `reg.taxonomy.*` namespaces pre-registered in `CANONICAL_NAMESPACES` (`event.rs` L309-315). Registry crate at `registry/templates/attack-taxonomy-mapper/` (manifest.yaml + 4 .j2 templates: select-evidence, map-taxonomy, taxonomize, convergence-check). SKILL.md at `.agents/skills/attack-taxonomy-mapper/SKILL.md`. Design spec at `docs/plans/attack-taxonomy-mapper-design.md`. OSC&R taxonomy verified against `github.com/pbom-dev/OSCAR` `matrix.json` (2026-07-18) — uses tactic + technique names, NOT numeric IDs. OWASP SC codes remain PROPOSED.

## Regulation Namespace Architecture (Point3 Research — `skill-logic-audit` + codebase)

**Finding:** `CANONICAL_NAMESPACES` is a flat array (`crates/hkask-types/src/event.rs`, L111-297). There is NO subgroup mechanism (`reg.skills` does not exist; `reg.skill` is a single namespace for skill lifecycle: activate/load/discover/publish/validate). Security spans are registered directly (`reg.tool.*`, `reg.inference`, `reg.fusion`, etc.).

**Recommendation (APPLIED):** `reg.supply_chain.select/probe/report/convergence`
are registered directly in `CANONICAL_NAMESPACES` (like `reg.inference`,
`reg.fusion`). NOT under a subgroup (`reg.skills.supply_chain` violates the
flat namespace design; `reg.skill.supply_chain` conflicts with `reg.skill`
lifecycle purpose).

**Evidence:** `SpanNamespace::new` validates against `CANONICAL_NAMESPACES`;
`is_canonical` checks byte-for-byte match; `scripts/check-reg-canonical.sh`
enforces this. The `skill-logic-audit` audit found the gap (`reg.supply_chain`
not registered) — the gap has since been CLOSED: all four namespaces
(`reg.supply_chain`, `reg.supply_chain.select`, `reg.supply_chain.probe`,
`reg.supply_chain.report`, `reg.supply_chain.convergence`) are registered at
