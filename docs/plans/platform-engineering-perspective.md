...
| **Brendan Gregg** (Netflix/Intel) | Systems performance engineering. Created the USE method (Utilization, Saturation, Errors), flame graphs, and the Linux performance observability toolkit. His insight: performance is a feature; you must measure everything. | hKask's Regulation VarietyTracker is saturation tracking. But where are utilization metrics? Where are flame-graph-equivalent traces of a FlowDef cascade through to inference? |

### 1.3 The Architects of Delivery

| Exemplar | Key Contribution | Relevance to hKask |
|----------|-----------------|-------------------|
| **Gene Kim** | Co-author of *The Phoenix Project*, *The DevOps Handbook*, and *Accelerate*. Codified the Three Ways of DevOps: Flow, Feedback, and Continual Learning. His insight: IT is not a cost center — it's a value stream. | hKask's kata system (PDCA cycles) is the Continual Learning way embodied. The Regulation is the Feedback way. But where is Flow? The time from a user's intent ("I need an agent that does X") to a running userpod is unmeasured. |
...
| Hightower | "Does a new user need to learn Regulation, OCAP, FlowDef, and rJoule just to create their first agent?" |
...
| **Strategy** | Aligns platform evolution with ecosystem co-evolution | Kata PDCA cycles, Curator metacognition, Regulation variety tracking |
...
| **Strategy** | Kata PDCA drives continuous capability development. Curator metacognition detects drift. Regulation variety tracking enforces Ashby's Law. | No market-facing strategy. hKask doesn't know whether it's competing with LangChain, Cursor, or Kubernetes. Wardley Mapping would reveal positioning. |
...
| **Radical transparency** | Architecture is documented. Failures are visible. The platform teaches you how it works. | Regulation spans emit at every membrane crossing. Self-healing is Regulation-audited. Deep-module justifications are public. |
...
|---------------|-------------------------------|-------------|
| **Provable sovereignty** | Cryptographic proof that user data has not been accessed without consent. Audit trail for every data access. | Partial — Regulation spans record operations, but there's no cryptographic receipt for the user. |
...
| **Governance transparency** | All platform decisions (Curator directives, spec changes, gas budget adjustments) are visible and explainable. | Strong — Regulation spans at every membrane crossing. Curator metacognition is documented. |
...
| **Co-evolution infrastructure** | Users can propose changes to the platform itself. Platform evolves with its ecosystem, not despite it. | Kata PDCA cycles invite user participation. Contract proposal Regulation spans exist (ContractProposed/Accepted/Rejected). |
...
| **Co-evolution is messy** | Letting users influence platform direction means saying "no" to good ideas and managing conflicting demands. | Governance clarity. The Magna Carta defines what can and cannot change. Contract proposal/rejection Regulation spans make decisions traceable. |
...
| **bug-hunt** | Run an expedition against the platform itself — not against agents, but against the platform's reliability. Find edge cases in FlowDef execution, Regulation span emission, OCAP enforcement. | Monthly |
...
| **superforecasting** | Calibrated probability forecasts: "What is the probability that Regulation alert fatigue becomes a problem by Q4 2026?" "Probability that a skill injection vulnerability is found in the wild within 12 months?" | Quarterly |
...
| **scenario-builder** | Scenario planning: "What if the primary inference provider goes down for 24 hours?" "What if a userpod gains write access to the skill registry?" "What if Regulation variety tracking saturates?" | Quarterly |
...
|-----------|------|----------|--------|---------|
| **platform-slo-evaluator** | FlowDef | Evaluate all registered SLOs against Regulation span data. Compute compliance, error budget remaining, burn rate. Emit `reg.slo.evaluated` spans. Classify breaches by severity. | SloDefinition registry, Regulation ν-event store | SloEvaluation per SLO, breach alerts |
| **platform-contract-auditor** | FlowDef | Run the CI contract check script. Parse compiler errors. Detect trait ↔ implementation drift across all port boundaries. Map each violation to the affected crate and port trait. | `cargo check` output, port trait registry | Contract violation report, Regulation spans |
| **platform-health-scorer** | KnowAct | Aggregate SLO compliance, test pass rate, contract violations, dependency health, Regulation variety metrics into a single platform health score (0.0–1.0). Track score over time. Identify degradation trends. | SloEvaluation list, test results, contract report, dependency graph | Health score (0.0–1.0), trend line, degradation alerts |
| **platform-dx-analyzer** | KnowAct | Analyze Regulation spans for developer experience signals. Time from sign-in to first agent creation. Skill creation frequency. Error resolution patterns. Identify friction points in the user journey. | Regulation spans (SessionOpen, AgentPod, Skill, Tool, SelfHeal) | DX report: time-to-first-agent, adoption rate, friction heatmap |
...
| **platform-bulkhead-auditor** | FlowDef | Identify failure domains. For each agent pod, skill, and MCP server: if this crashes, what else breaks? Map blast radius. Recommend bulkheads where blast radius > 1 component. | Crate dependency graph, Regulation spans for each loop membrane | Bulkhead audit: blast radius per component, recommendations |
| **platform-consent-auditor** | FlowDef | Audit every DelegationToken issued. Report: who issued it, to whom, for what resource, with what action, when does it expire, has it been used. Flag anomalous patterns (e.g., token with Critical severity issued outside working hours). | DelegationToken registry, Regulation spans | Consent audit report, anomaly alerts |
...
| **platform-governance-transparency-reporter** | KnowAct | Generate a human-readable report of all platform governance decisions in the last N days. Curator directives, spec changes, gas budget adjustments, contract proposals/acceptions/rejections. Explain each decision in plain language. | Regulation spans (Curation, Spec, ContractProposed/Accepted/Rejected, CuratorDirective) | Governance transparency report |
...
This loop is what lock-in platforms cannot replicate. They can measure retention but not loyalty. They can count users but not trust. hKask's architecture — Magna Carta principles, Regulation observability, OCAP boundaries, P2 consent — makes loyalty *measurable* for the first time.
...
| DORA Concept | Skill Equivalent | Regulation Span |
|-------------|-----------------|----------|
| How often do you deploy? | How often does a skill get created or updated in the registry? | `reg.skill` (created, loaded, activated) |
...
**Current state:** Regulation emits `reg.skill` spans when skills are loaded, discovered, or activated. The count is knowable but not measured as a delivery metric.
...
| DORA Concept | Skill Equivalent | Regulation Span |
|-------------|-----------------|----------|
| Time from commit to production | Time from YAML/J2 file save to first successful skill dispatch in a live session | `reg.skill` → `reg.tool.skill_dispatch` |
...
Each step is Regulation-observable. The gap between them is measurable but not currently computed.

**Current state:** Not measured. The Regulation knows when a skill is loaded and when it's dispatched, but doesn't compute the latency between them per skill.
...
| DORA Concept | Skill Equivalent | Regulation Span |
|-------------|-----------------|----------|
| % of deployments causing failure | % of skill dispatches that error (render failure, FlowDef validation failure, cascade abort, timeout) for skills changed within the last N days | `reg.tool.skill_dispatch` (with error field, filtered by `changed_since`) |
...
| DORA Concept | Skill Equivalent | Regulation Span |
|-------------|-----------------|----------|
| Time to restore service | Time from first dispatch error on a changed skill to the next successful dispatch of that same skill | `reg.tool.skill_dispatch` (error → next success, correlated per skill) |
...
**Current state:** Not measured. Regulation knows when dispatches succeed and fail but doesn't correlate them per skill to compute recovery time.

**Elite target for hKask skills:** < 1 hour from first failed dispatch to next successful dispatch.

#### Regulation Span Availability for DORA

| DORA Metric | Required Regulation Data | Already Emitted? | Gap |
|-------------|------------------|-----------------|-----|
| Deployment Frequency | `reg.skill` (created/updated timestamps) | ✅ Yes | Aggregation only |
| Lead Time | `reg.skill.created` → `reg.tool.skill_dispatch` (first success per skill) | ⚠️ Partial — both spans exist | Correlation per skill |
| Change Failure Rate | `reg.tool.skill_dispatch` with error + skill version tracking + "changed recently" filter | ⚠️ Partial — dispatch spans exist | Version tracking + time filter |
| MTTR | `reg.tool.skill_dispatch` (error → next success per skill) | ⚠️ Partial — both spans exist | Correlation per skill |

**Key insight:** The data is already there. All four DORA metrics can be computed from existing Regulation spans. What's missing is the **aggregation pipeline** — a FlowDef that queries the ν-event store, correlates spans per skill, computes the four metrics, and emits `reg.platform.metric` spans with the results. This is exactly what `platform-dx-analyzer` would do.
...
#### SPACE Dimensions Mapped to Regulation Spans

| SPACE Dimension | Regulation Spans Used | Aggregation |
|----------------|---------------|-------------|
| **S**atisfaction | Survey response (stored as episodic memory), `reg.session` (session frequency as proxy for engagement) | NPS calculation, engagement trend |
| **P**erformance | `reg.tool.skill_dispatch` (latency, success/failure), `reg.skill` (adoption by other users) | SLO compliance, dispatch latency p50/p95/p99, adoption count per skill |
| **A**ctivity | `reg.skill` (created/updated), `reg.tool.skill_dispatch` (usage count), `reg.session` (session count) | Counts per user per week/month |
| **C**ommunication | `reg.skill` (shared/public flag), FlowDef composition graph (skill A calls skill B), skill import graph | Shared skill count, composition depth, external usage rate |
| **E**fficiency | `reg.skill.created` → `reg.tool.skill_dispatch` (first success latency), error-to-success cycle count per skill change | Lead time, edit-test cycle count, session duration |
...
steps:
  - name: compute_dora_metrics
    ...
    description: >
      ...
      Emit reg.platform.metric spans for each DORA metric per user.
    inputs:
      window_days: 30
  ...
  - name: compute_space_dimensions
    ...
    description: >
      ...
      Emit reg.platform.metric spans for each SPACE dimension per user.
    inputs:
      window_days: 30
    outputs:
...
### 5.4 DORA/SPACE Dashboard Regulation Spans

New Regulation spans for DORA/SPACE tracking:

| Span | Description | Fields |
|------|------------|--------|
| `reg.platform.metric.dora.deploy_freq` | Skill deployment frequency per user | `user`, `count`, `window_days`, `cluster` |
| `reg.platform.metric.dora.lead_time` | Skill lead time per user | `user`, `p50_ms`, `p95_ms`, `window_days`, `cluster` |
| `reg.platform.metric.dora.change_fail_rate` | Skill change failure rate per user | `user`, `rate`, `total_changes`, `failed_changes`, `window_days`, `cluster` |
| `reg.platform.metric.dora.mttr` | Skill mean time to recovery per user | `user`, `p50_ms`, `p95_ms`, `window_days`, `cluster` |
| `reg.platform.metric.space.satisfaction` | SPACE Satisfaction score | `user`, `nps`, `confidence_score`, `window_days` |
| `reg.platform.metric.space.performance` | SPACE Performance score | `user`, `slo_compliance`, `adoption_rate`, `window_days` |
| `reg.platform.metric.space.activity` | SPACE Activity score | `user`, `skills_created`, `skills_updated`, `dispatches`, `window_days` |
| `reg.platform.metric.space.communication` | SPACE Communication score | `user`, `skills_shared`, `composition_depth`, `external_usage`, `window_days` |
| `reg.platform.metric.space.efficiency` | SPACE Efficiency score | `user`, `idea_to_dispatch_p50`, `edit_test_cycles_avg`, `window_days` |
...
|---------|-------|-----------------|
| **Hexagonal Architecture** | `hkask-ports` — trait abstractions for Regulation, inference, embedding, tool dispatch, registry, git-cas, federation | Infrastructure swappable without touching domain logic |
...
| **Self-Healing** | SelfHealer on every fallible operation, 6 built-in strategies, full Regulation audit trail | Autonomous recovery as first resort |
| **Deep Module Discipline** | ≤7 public items per crate, deletion test justification for all crates | API surface minimalism |
| **Property-Based + Fuzz + Mutation Testing** | cargo-bolero, cargo-mutants, state-machine roundtrip, Regulation span contract fuzzing, LLM QA triage | Testing as verification, not coverage-counting |
...
| **Kata Improvement Loop** | PDCA cycles, coaching 5-question dialogue, Regulation span trace per experiment | Continuous improvement as first-class process |

### 7.2 Identified Gaps

| Gap | PE Concern | Severity | User Impact |
|-----|-----------|----------|-------------|
| **No explicit SLOs** | Reliability Engineering | High | Regulation detects anomalies but has no user-facing contracts |
...
| L2 Operational | ⚠️ Partial — Regulation automates regulation, gap docs acknowledge gaps |
| L3 Scalable | ✅ Skills are self-service, FlowDef templates, Regulation tracks variety |
...
```
INVESTMENT 1 ── SLOs wired to Regulation
...
## 9. Investment 1 — SLOs Wired to Regulation
...
| SLO ID | Name | Regulation Span | Target | Window | Severity |
|--------|------|----------|--------|--------|----------|
| SLO-INF-001 | Inference availability | reg.inference.* | 99.9% success | 30d | Critical |
| SLO-INF-002 | Inference p95 latency | reg.inference.duration_ms | < 5,000ms | 7d | High |
| SLO-SKL-001 | Skill dispatch success | reg.tool.skill_dispatch | 99.5% | 30d | Critical |
| SLO-SKL-002 | Skill dispatch p95 latency | reg.tool.skill_dispatch.duration_ms | < 2,000ms | 7d | High |
| SLO-Regulation-001 | Regulation algedonic delivery | reg.algedonic.* | 99.9% within 30s | 30d | Critical |
| SLO-MEM-001 | Memory consolidation | reg.memory.consolidation | 99.0% | 7d | High |
| SLO-CUR-001 | Curator escalation response | reg.curation.escalation | < 60s p95 | 7d | Medium |
| SLO-API-001 | API endpoint availability | reg.api.* | 99.9% | 30d | Critical |
| SLO-WLT-001 | Wallet operation success | reg.wallet.* | 99.99% | 30d | Critical |
...
### 9.3 Regulation Integration

New types: `SloDefinition`, `SloSeverity` (Critical/High/Medium), `SloEvaluation`.

New Regulation span: `reg.slo.evaluated` — emitted per evaluation cycle with `slo_id`, `current_compliance`, `error_budget_remaining`, `burn_rate`.
...
| Metric | Definition | Regulation Span | Cadence |
|--------|-----------|----------|---------|
| **Time to First Agent** | Wall-clock time from sign-in to first successful agent creation | reg.onboarding.complete → reg.agent.created | Per user |
| **Time to 10th Skill** | Wall-clock time from first skill creation to 10th | reg.skill.created | Per user |
| **Skill Adoption Rate** | % of created skills used in ≥3 sessions within 30 days | reg.tool.skill_dispatch | Monthly |
| **Platform NPS** | Prompt-based survey in REPL after 10th session | N/A (survey) | Quarterly |
| **Active User Retention** | % of users active in both current and previous 30-day windows | reg.session.* | Monthly |
| **Error Resolution Time** | Time from Regulation alert to SelfHealer resolution or human intervention | reg.algedonic.* → reg.heal.* | Per incident |

### 10.2 Regulation Integration

New Regulation span: `reg.platform.metric` — emitted per metric evaluation with `metric_name`, `value`, `window`, `trend`.
...
|--------|-------|-----------|
| **Read** | Regulation spans, SLO evaluations, dependency graph, crate public surfaces, test results | Direct via service layer |
| **Signal** | Recommendations to Curator, SLO breach alerts, health score changes | Regulation spans + CuratorDirective |
| **Write** | Platform health reports (read-only hMem), metric evaluations | EpisodicMemory via OCAP |
| **Never** | Source code, configuration files, deployment, agent definitions, wallet operations | Enforced by capability membrane |

### 11.4 Regulation Integration

New Regulation spans:
- `reg.platform.audit.started` — Platform audit cycle begins
- `reg.platform.audit.completed` — Audit cycle complete with findings
- `reg.platform.recommendation` — UserPod proposes an intervention
- `reg.platform.recommendation.accepted` — Human curator accepts
- `reg.platform.recommendation.rejected` — Human curator rejects (with reason)
...
| **B: Regulation Feedback Loop** | SLO breach is a new algedonic trigger; PaaP metrics are new Regulation spans; Platform Engineer userpod is a new observer |
...
| **Curation** | Platform Engineer userpod reports to Curator. New Regulation spans for audit/recommendation lifecycle |
...
| Phase | What | Status |
|-------|------|--------|
| **Phase 1** | SloDefinition type + 3 seed SLOs + `reg.slo.evaluated` Regulation span | ✅ Done |
| **Phase 2** | SloManager in hkask-regulation + RegulationLedger integration + `evaluate_and_escalate_slos()` wired to algedonic pathway + SLO evaluation scheduled in CyberneticsLoop tick | ✅ Done |
| **Phase 3** | PaaP metric Regulation spans + API | ✅ Done — 11 Regulation spans added. `platform-dx-analyzer` FlowDef manifest registered. API endpoints for metric queries deferred (not in scope for platform engineering infrastructure). |
| **Phase 4** | DORA/SPACE Regulation spans + `platform-dx-analyzer` FlowDef | ✅ Done — Regulation spans + FlowDef manifest + template crate + compute-dora KnowAct template. Registered in bootstrap and startup. |
...
| **Phase 6** | Sovereignty skills (consent-auditor, portability-verifier, governance-transparency-reporter) | ✅ Done — 3 FlowDef manifests + 3 template crate manifests + 6 KnowAct templates + `TokenRegistry` trait + SQLite `TokenRegistryStore` + `list_tokens` MCP tool + 5 sovereignty Regulation spans |
...
|-------|----------|---------------|
| `hkask-types` | — | `reg.rs` (+16 RegulationSpan variants, +SloDefinition, +SloSeverity, +SloEvaluation, +seed_slos) |
| `hkask-regulation` | `slo_manager.rs` | `runtime.rs`, `cybernetics_loop.rs`, `lib.rs` |
...
|-----------|------------|--------|
| SLOs are defined and tracked | `reg.slo.evaluated` span count | ≥9 SLOs active within 30 days of Phase 1 start |
| Error budgets inform decisions | % of SLO breaches that trigger an intervention | >80% within 60 days |
| Platform metrics are measurable | PaaP metric Regulation spans emitted | All 6 metrics emitting within 30 days of Phase 3 start |
| DORA metrics are tracked per user | `reg.platform.metric.dora.*` spans | All 4 DORA metrics emitting within 30 days of Phase 4 start |
| SPACE dimensions are tracked per user | `reg.platform.metric.space.*` spans | All 5 SPACE dimensions emitting within 30 days of Phase 4 start |
| Platform Engineer userpod is active | `reg.platform.audit.*` spans | Weekly audits running within 30 days of Phase 5 start |
...
| Loyalty scorecard is operational | `reg.platform.metric.loyalty` span count | Monthly loyalty scores emitting within 30 days of Phase 7 start |
...
| PaaP metrics gamed | Low | Medium | Metrics anchored in Regulation spans — hard to fake without system compromise |
