...
### F-L2-003 — `DelegationToken::expires_at` enforcement is unverified by a negative test (Q3)
...
### F-L2-005 — `MemoryStoragePort` is a port but not a capability — the boundary is the constructor, not the type (Q4)
