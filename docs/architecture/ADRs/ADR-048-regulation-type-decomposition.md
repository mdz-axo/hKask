...
**Chosen Approach:** Decompose the monolithic `RegulationSpan` into a layered architecture:

1. **Core (hkask-types):** `RegulationSpan` reduced to 7 cross-cutting variants (Tool, Inference, AgentPod, Gas, Curation, SelfHeal, MemoryEncode) — spans constructed in 2+ crates from different dependency domains.
...
1. **Single `Domain(&'static str)` variant** — Keeps RegulationSpan as one enum with an extensible string variant. Simpler (zero new types) but loses per-domain compile-time type safety. Rejected: the user explicitly wanted "subtypes in the crates."
2. **Keep monolithic RegulationSpan** — No change. Rejected: violates substrate isolation principle; 72+ variants in a substrate crate.
...
### Negative
- Adding a new span requires 3 changes (enum variant + CANONICAL_NAMESPACES + call site) vs. 1 before (just RegulationSpan variant)
- `RegulationSpan::from_str()` no longer accepts domain namespace strings — any code using it for parsing must switch to `SpanNamespace::parse()`
...
- `SpanNamespace::from_observable()` and `From<RegulationSpan>` are structurally identical — deduplicated through shared `from_str_validated()` helper
...
|-----------|-----------|----------|
| **P1** (No trait without two consumers) | ✅ | `ObservableSpan` has 10 impls (RegulationSpan, WalletSpan, FederationSpan, 7 subsystem spans) |
| **P3** (No module directory without encapsulation) | ✅ | Each span file encapsulates a single subsystem's observability |
| **P6** (Delete stubs, don't publish) | ✅ | 65 removed RegulationSpan variants deleted, not deprecated |
| **P7** (Prefer deletion over deprecation) | ✅ | Old RegulationSpan variants removed entirely; domain enums replace them |

### Constraints

| Constraint | Compliance | Evidence |
|-----------|-----------|----------|
| **C4** (Repetition is missing primitive) | ✅ | `from_str_validated()` extracted as the single validation path for both `From<RegulationSpan>` and `from_observable()` |
| **C7** (Divergence must yield) | ✅ | Split brain between `RegulationSpan::from_str()` and `CANONICAL_NAMESPACES` resolved — single registry |
...
grep -rn 'RegulationSpan::' crates/ --include='*.rs' | grep -v 'reg_span\|reg.rs\|Tool\|Inference\|AgentPod\|Gas\|Curation\|SelfHeal\|MemoryEncode'  # should find nothing
```
...
- Zero stray references to removed RegulationSpan variants
