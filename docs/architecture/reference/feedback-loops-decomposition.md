...
:CapabilityAttenuation_Loop  a                :FeedbackLoop ;
    ...
    :hasFeedbackPath    "cns.tool.unauthorized → RevocationStore" ;
...
**Implementation status:** ✅ Implemented — `CapabilityToken` with HMAC-SHA256, `attenuate()`, `verify_cryptographic()`, `OCAP` struct with `AttenuationHistory`, `RevocationStore` (SQLite), `SecurityGateway` with audit log, `CapabilityAwareValidator` for template gating.
...
### Loop 4: The Cybernetics Loop

> **Note:** In the 6-loop architecture (3 domain + 3 meta), the former Governance and Observability loops have been merged into this single Cybernetics loop as a meta loop. This reflects that governance (OCAP enforcement, capability attenuation) and observability (variety tracking, algedonic alerts) are two aspects of the same homeostatic self-regulation cycle.

**Purpose:** Detect variety deficit, enforce OCAP governance, and escalate to Curator/human when the system cannot self-regulate.
...
```
:Cybernetics_Loop  a                :FeedbackLoop ;
    :hasInput           "NuEvent (Phase::Observe)" ;
    :hasOutput          "RuntimeAlert → EscalationAction" ;
    :hasEvaluation      "VarietyTracker + AlgedonicManager + OCAP enforcement" ;
...
:Cybernetics_Loop  :span              "cns.variety.* / cns.killzone.* / cns.energy.* / cns.tool.*" ;
    :threshold          "deficit > 100 (default)" ;
    :severityScale      "Info ≤50, Warning 51-100, Critical >100" .
```

**Implementation status:** ✅ Fully implemented — `CnsRuntime` with `AlgedonicManager`, `VarietyTracker`, `VarietyMonitor`, `BotMetricsCollector`, `SovereigntyObserver`, `AlgedonicEscalationAdapter`, `EnergyBudget`, `GoalVarietyMonitor`, `SecurityGateway` (OCAP enforcement). ⚠️ Gap: automated remediation is Curator-directed only (`applied: false` until `mark_applied()`); no autonomous remediation loop.
...
| Cybernetics | Variety counter vs. threshold (100/500) + OCAP enforcement | `VarietyTracker::deficit()` + `AlgedonicManager::check()` + `BotMetricsCollector` + `SecurityGateway` | `CalibrateThreshold` / `TriggerKata` / `EscalateToHuman` via `AlgedonicEscalationAdapter`; attenuation narrows scope |
...
```
Cybernetics_Loop  entryPoint       CnsEmit trait / SpanEmitter / SecurityGateway
Cybernetics_Loop  attenuation      SpanScope (allowed_categories per domain) + CapabilityToken attenuation
Cybernetics_Loop  revocation       Out-of-scope emission → cns.sovereignty.alert.boundary_violation; RevocationStore for tokens
Cybernetics_Loop  auditTrail       cns.variety.* / cns.tool.* → AlgedonicManager → RuntimeAlert → EscalationAction
```

**Membrane entry:** `CnsEmit` trait — all telemetry flows through `emit_event(span, phase, observation, confidence)`. `SecurityGateway` gates capability-protected operations.
**Attenuation:** `SpanScope` restricts emission categories per domain (e.g., `mcp` can only emit `Tool` category). Out-of-scope emissions trigger sovereignty violation. `CapabilityToken.attenuate()` narrows authority per OCAP.
**Revocation:** Sovereignty violation alerts are emitted and tracked by `SovereigntyObserver`; threshold violations trigger algedonic escalation. `RevocationStore` invalidates capability tokens.
**Audit:** `NuEvent` records with `Phase::Observe/Regulate/Outcome`; `VarietyTracker` accumulates; `AlgedonicManager` checks deficit; `SecurityGateway.audit()` logs capability operations.
...
| Cybernetics | `hkask-cns` + `hkask-mcp` | `NuEvent`, `Span`, `Phase`, `VarietyTracker`, `VarietyMonitor`, `AlgedonicManager`, `RuntimeAlert`, `AlertSeverity`, `CnsHealth`, `BotMetricsCollector`, `SovereigntyObserver`, `SecurityGateway` | `CnsRuntime::check_variety()`, `AlgedonicManager::check()`, `SpanEmitter::emit()`, `CnsRuntime::subscribe()`, `AlgedonicEscalationAdapter::process_alert()`, `SecurityGateway::verify_capability()` | `CnsEmit`, `NuEventSink` |
...
| Cybernetics | ✅ Yes | `VarietyTracker.deficit()` vs. threshold + `SecurityGateway.verify_capability()` | ✅ Yes | `CnsRuntime.subscribe()` → escalation callbacks + `SecurityGateway.audit()` → `cns.tool.*` | ⚠️ Partial | Escalation produces `EscalationAction` but `applied: false` until Curator acts; attenuation narrows scope |
...
| Cybernetics | NuEvent emitted (Phase::Observe) | Variety deficit computed, algedonic check, OCAP enforcement | Alert produced, escalation action, capability attenuation | `Span::Variety` / `Span::Tool` |
...
graph TD
    ...
    subgraph "Cybernetics Loop"
        OBS_OBS["Observe:<br/>NuEvent emitted"] --> OBS_REG["Regulate:<br/>VarietyTracker,<br/>AlgedonicManager,<br/>SecurityGateway"]
        OBS_REG --> OBS_OUT["Outcome:<br/>RuntimeAlert,<br/>EscalationAction,<br/>capability attenuation"]
    end
...
:Cybernetics_Loop a :FeedbackLoop ;
    ...
    :hasEvaluation "VarietyTracker + AlgedonicManager + SecurityGateway (OCAP)" ;
    :hasFeedbackPath "cns.* → VarietyCounter → AlgedonicManager → CnsRuntime.subscribe()" ;
    :hasImprovement :EscalationAction ;
    :instantiates :ObserveRegulateOutcome ;
    :threshold 100 ;
    :span "cns.variety.* / cns.killzone.* / cns.energy.* / cns.tool.*" .
...
:SpecificationCuration_Loop :feedsInto :Cybernetics_Loop ;
    :connection "cns.spec.drift → variety deficit" .
:Cybernetics_Loop :feedsInto :PerceptionAction_Loop ;
    :connection "algedonic alert → Curator → revised template selection" .
:Cybernetics_Loop :feedsInto :PodLifecycle_Loop ;
    :connection "capability gap → BotDirective" .
:Cybernetics_Loop :feedsInto :Sovereignty_Loop ;
...
:GoalLifecycle_Loop :feedsInto :CapabilityAttenuation_Loop ;
    :connection "GoalCapabilityToken subtypes CapabilityToken" .
:PodLifecycle_Loop :feedsInto :Cybernetics_Loop ;
...
:EnergyBudget_Loop :feedsInto :Cybernetics_Loop ;
