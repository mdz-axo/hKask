...
| `hkask-inference` | 18 | InferenceRouter, provider backends (DeepInfra, Together AI, fal.ai, RunPod, Baseten), model catalog, budget tracking | Provider abstraction layer. Each provider backend is a concern; the router composes them. Backend count scales with provider support. |
