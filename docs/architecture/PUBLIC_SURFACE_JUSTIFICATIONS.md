...
| `hkask-inference` | 18 | InferenceRouter, provider backends (DeepInfra, Fireworks, Ollama, fal.ai), model catalog, budget tracking | Provider abstraction layer. Each provider backend is a concern; the router composes them. Backend count scales with provider support. |
