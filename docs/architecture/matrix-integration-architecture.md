...
| **Sidecar total (with Hydrogen)** | ~55 MB (Conduit ~50 MB + Hydrogen ~5 MB) | ~700 MB (Synapse ~500 MB + Postgres ~200 MB + Element Web ~200 MB) |
...
1. **Container status:** Runs `docker compose -f ~/.config/hkask/sidecar/docker-compose.yml ps --format json` and parses container state.
...
4. **Hydrogen health:** `GET http://localhost:8080` — verifies the web server responds (only if Hydrogen profile is active).
...
```
1. Admin deploys Conduit sidecar:
   kask matrix deploy-sidecar --domain matrix.example.com
   → Generates ~/.config/hkask/sidecar/docker-compose.yml + conduit.toml
   → Admin runs: docker compose -f ~/.config/hkask/sidecar/docker-compose.yml up -d

2. Admin configures TLS (REQUIRED — FluffyChat will not connect to plain HTTP):
   Option A: Reverse proxy on the server (nginx/caddy + Let's Encrypt)
   Option B: Cloudflare Tunnel (if domain is on Cloudflare)
   Option C: Traefik sidecar container added to docker-compose
   → Conduit's port 8008 is proxied to https://matrix.example.com
   → Conduit's federation port 8448 is proxied (if federation enabled)
   → /.well-known/matrix/client serves JSON pointing to https://matrix.example.com

3. Admin enables user registration on Conduit:
   → Edit conduit.toml: allow_registration = true
   → Or keep registration closed and use: kask matrix register --user bob
     (hKask creates the account via Conduit's admin API, outputs password)
...
│  │  localhost:8008       │  │                          │ │
│  │                      │  │  Daemon (daemon.sock)     │ │
│  │  Docker: Hydrogen     │  │  Curator (Curation Loop) │ │
