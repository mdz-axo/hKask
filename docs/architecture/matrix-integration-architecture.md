...
**hKask does not maintain Conduit, Hydrogen, or Caddy.** It generates configuration files and docker-compose orchestration. The containers are upstream images, pulled by the user. hKask's responsibility ends at config generation and health-check tooling (`kask matrix status-sidecar`).
...
| `allow_registration` | `true` | Admin can disable after human accounts are created |
| `allow_federation` | `false` | Opt-in only — admin changes to `true` if needed |
| `admin_token` | Random 64-char hex string | Stored in `hkask-keystore`; used by `kask matrix register` |
| `well_known_client` | `https://matrix.example.com` | Served by Conduit, proxied by Caddy |
