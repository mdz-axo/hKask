...
| Q6 | Task 3, O1 | **Nested Arc<RwLock<Arc<RwLock<...>>>>:** `SessionManager.chats` uses double locking. Is this justified by concurrent access patterns, or can `EnsembleChat` own its state without interior mutability? **Resolved (2026-06-14):** Ensemble crate removed. Question preserved as design reference for future multi-agent mode evolving from dual-presence. | [Evidence, IS-Probabilistic] | [Directly Stated, source code observation] | N/A — ensemble deferred. Revisit when dual-presence produces N≥3 stable sessions with distinct ACP agents. | **Resolved** — ensemble deferred |
...
|---|----------|---------------|------------|-------------------|-----------------|
| Q17 | **CNS algedonic alert persistence during idle:** Should alerts be persisted to storage and replayed when Curator becomes active, or should a background consumer process alerts even when no chat session is active? | [Hypothesis, IS-Subjunctive] | [Directly Stated, Task 4 deferral of R1] | **RESOLVED:** The algedonic system must always be connected to the Curator replicant/agent. Implementation direction: spawn CuratorAgent's algedonic receiver as a background task with lifetime independent of chat sessions. The Curator is the designated consumer of all algedonic signals — an architectural invariant. | **Guideline** — resolved, implement background consumer |
