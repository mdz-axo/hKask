---
title: "Crate Audit — Task 4: Surgical Improvement Implementation"
...
|----|---------|-----------------|-----------------|
| **R1** | CNS closure break during idle (algedonic alerts with no consumer) | Guardrail | **FIXED:** Merged the two separate alert-delivery blocks in `CyberneticsLoop::act()` into a single fallback chain: live `alerts_tx` channel first, `event_sink` persistence second. When the Curator's inbox is unavailable (no active session), alerts are now persisted to `NuEventStore` with full alert data (`deficit`, `threshold`, `current`, `timestamp`). The CurationLoop's existing `restore_cursor()` mechanism can retrieve persisted alerts on next activation. Per design decision: the algedonic system must always be connected to the Curator — persistence is the bridge when the live channel has no receiver. Changed file: `hkask-cns/src/cybernetics_loop.rs` (lines 510–558). Verification: `cargo check` ✓, `cargo test` (11/11) ✓. |
