...
The dual-presence pattern — where two entities (a sovereign host userpod and a co-participant daemon) share one CLI/REPL conversation loop — requires five architectural decisions:
...
| **C — Sender-weighted** | Weight by sender identity (human owner > userpod > bot > unknown) | Respects sovereignty hierarchy | Requires sender identity verification |
...
**Context:** 8 route files in `crates/hkask-api/src/routes/` lack `#[utoipa::path]` annotations: `admin.rs`, `auth.rs`, `export.rs`, `landing.rs`, `pods.rs`, `userpod.rs`, `settings.rs`, `terminal.rs`. This means 33% of routes are absent from the generated `openapi.json`.
