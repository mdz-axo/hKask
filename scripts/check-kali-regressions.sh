...
#
# Exit codes:
#   0 — all enforced regressions pass (no bug patterns found)
#   1 — an enforced regression's bug pattern was found (re-introduction)
#
...
for rr_file in "$REGRESSIONS_DIR"/RR-*.yaml; do
  ...
  # Only grep-kind regressions are mechanically enforced.
  if [ "$rr_kind" != "grep" ]; then
    continue
  fi
...
echo "summary: $violations violation(s), $enforced enforced, $pending pending"
